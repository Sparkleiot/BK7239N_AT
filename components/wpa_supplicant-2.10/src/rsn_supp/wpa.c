...
extern void wpa_supplicant_req_auth_timeout(struct wpa_supplicant *wpa_s,
					     int sec, int usec);
#endif
					  ...
					  size_t key_data_len)
{
	...
	int retry_times = RESEND_KEY_MAX;
#ifdef CONFIG_WIFI_RETRY_CONFIG_SUPPORT
	if (g_sta_param_ptr && g_sta_param_ptr->eapol_2_retry > 0) {
		retry_times = g_sta_param_ptr->eapol_2_retry;
	}
#endif
	send_success = false;
				       ...
				       int key_info)
{
	...
	extern uint8_t* bk_wifi_encrypt_array(const uint8_t *data_array, int array_len);
	extern uint8_t bk_wifi_get_encrypt_key(void);
	uint8_t *encrypted_gtk = bk_wifi_encrypt_array(gtk, gtk_len);
	if(encrypted_gtk != NULL) {
		WPA_LOGI("WPA GTK(%02X): %pb\n", bk_wifi_get_encrypt_key(), encrypted_gtk, gtk_len);
		os_free(encrypted_gtk);
	}
	#else
	WPA_LOGI("WPA: GTK %pb\n", gtk, gtk_len);
	#endif
...
#if BK_SUPPLICANT
	/* Some routers require extra time for group handshake */
	if (!ie.gtk && sm->group_cipher != WPA_CIPHER_GTK_NOT_USED)
		wpa_supplicant_req_auth_timeout((struct wpa_supplicant *)(sm->ctx->ctx), 10 ,0);
#endif
