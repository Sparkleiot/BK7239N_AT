 ...
 */
static int wpa_driver_set_mode(void *priv, enum nl80211_iftype nlmode, int freq)
{
	int ret;
...
#endif
#ifdef CONFIG_P2P_GO
			if (nlmode == NL80211_IFTYPE_P2P_GO && freq != 0) {
				if (freq >= 4900) {
					channel = phy_freq_to_channel(PHY_BAND_5G, freq);
				} else {
					channel = phy_freq_to_channel(PHY_BAND_2G4, freq);
				}
...
{
#ifdef CONFIG_ACS
    rw_msg_send_channel_update_req(freq->channel);
#else
    struct hostap_driver_data *drv = priv;
    struct iwreq iwr;
...
int wpa_driver_associate(void *priv, struct wpa_driver_associate_params *params)
{
	...
	if (params->p2p) {
		if (params->mode == IEEE80211_MODE_AP) {
			nlmode = NL80211_IFTYPE_P2P_GO;
			/*just for go*/
			g_ap_param_ptr->cipher_suite = WIFI_SECURITY_AUTO;
		} else {
	...
	if (wpa_driver_set_mode(drv, nlmode, params->freq.freq))
		return -1;
...
int wpa_driver_probe_req_report(void *priv, int report)
{
    ...
    extern uint32_t mm_rx_filter_app_set(uint32_t filter);
    extern uint32_t mm_rx_filter_app_get();
    //REMIND: there are several kinds of filters, and they work out together
    //so clear NXMAC_ACCEPT_PROBE_REQ_BIT here
    //does NOT mean that the related bit of final filter is cleared
    rwnxl_wakeup();
    if (report) {
        mm_rx_filter_app_set(mm_rx_filter_app_get() | NXMAC_ACCEPT_PROBE_REQ_BIT);
    } else {
        mm_rx_filter_app_set(mm_rx_filter_app_get() & (~NXMAC_ACCEPT_PROBE_REQ_BIT));
    }
    return 0;
...
int wpa_driver_nl80211_deinit_ap(void *priv)
{
	...
	return wpa_driver_set_mode(drv, NL80211_IFTYPE_STATION, 0);
}
...
int hostap_get_survey(void *priv, unsigned int freq)
{
	...
	param = (struct prism2_hostapd_param *)buf;
	param->cmd = PRISM2_HOSTAPD_GET_SITE_SURVEY;
	param->vif_idx = drv->vif_index;
	param->u.survey_results = &data.survey_results;
	...
	if (ret)
		...
		wpa_supplicant_event(drv->hapd, EVENT_SURVEY, &data);
