...
#if CONFIG_BRIDGE
struct wpabuf * wpa_bss_get_bridge_vendor_ie(const struct wpa_bss *bss,
					    u32 vendor_type)
{
	struct wpabuf *buf;
	const u8 *end, *pos;
	...
	while (end - pos > 1) {
		...
		if (len > end - pos - 2)
			break;
		pos += 2;
		if (ie == WLAN_EID_VENDOR_SPECIFIC && len >= 8 &&
		    !os_memcmp(pos, "\x74\x79\x62\x72\x69\x64\x67\x65", 8))
			wpabuf_put_data(buf, pos, len - 2);
		pos += len;
	}
