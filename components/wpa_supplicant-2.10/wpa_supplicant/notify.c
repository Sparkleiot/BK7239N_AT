...
void wlan_store_fci(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->pairwise_cipher > WPA_CIPHER_NONE) {
		...
		if (ssid->psk_set) {
			psk = ssid->psk;
			for (i = 0; i < PMK_LEN; i++) {
				sprintf((char*)fci.psk + i * 2, "%02x", psk[i]);
			}
		}
	} else {
...
#if BK_SUPPLICANT
bool wpas_check_if_6mbps_supported(struct wpa_supplicant *wpa_s)
{
	struct wpa_bss *bss = wpa_s->current_bss;
	u8 *rates = NULL;
	int num_rates;
	int i;
	bool rate_6m_supported = false;

	if (bss != NULL) {
		/* Get supported rates from BSS (units of 500 kbps) */
		num_rates = wpa_bss_get_bit_rates(bss, &rates);
		if (num_rates > 0 && rates != NULL) {
			/* Check if 6Mbps rate is supported (6Mbps = 12 * 500kbps) */
			for (i = 0; i < num_rates; i++) {
				if (rates[i] == 12) {  /* 12 = 6Mbps in 500kbps units */
					rate_6m_supported = true;
					break;
				}
			}
			os_free(rates);
...
void wpas_notify_connected(struct wpa_supplicant *wpa_s)
{
	if (mhdr_get_station_status().state < WIFI_LINKSTATE_STA_CONNECTED) {
		...
		/* Check if router supports 6M rate, if yes, set ARP keepalive rate to 6M */
		if (wpas_check_if_6mbps_supported(wpa_s)) {
			/* Set ARP keepalive rate to 6M (60 = 6Mbps in 100kbps units) */
			bk_wifi_send_arp_set_rate_req(60);
		}
#endif
			       ...
			       enum wpa_states old_state)
{
	...
	if (new_state == WPA_DISCONNECTED && old_state > WPA_SCANNING) {
		...
		if ((old_state == WPA_COMPLETED && wpa_s->disable_auto_reconnect_after_disconnect) || /* don't reconnect */
			...
			{
				if (g_sta_param_ptr->cipher_suite == WIFI_SECURITY_WPA3_WPA2_MIXED ||
					g_sta_param_ptr->cipher_suite == WIFI_SECURITY_WPA3_SAE)
				{
					wpa_s->disconnect_reason = WIFI_REASON_WRONG_PASSWORD;
				}
				//else if(g_sta_param_ptr->cipher_suite >= WIFI_SECURITY_WPA_TKIP &&
				//	g_sta_param_ptr->cipher_suite <= WIFI_SECURITY_WPA2_MIXED)
				//{
				//	wpa_s->disconnect_reason = WIFI_REASON_AUTH_FAIL;
				//}
				//else if(g_sta_param_ptr->cipher_suite == WIFI_SECURITY_NONE)
				//{
				//	wpa_s->disconnect_reason = WIFI_REASON_AUTH_FAIL;
				//}
			}
...
void wpas_notify_sae_password_mismatch(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_INFO, "%s: SaePasswordMismatch\n", __func__);
