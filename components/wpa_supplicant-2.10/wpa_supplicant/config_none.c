...
int wpa_config_set_none(struct wpa_ssid *ssid)
{
	...
	ssid->mem_only_psk = 0;
	ssid->auth_alg = 0;
	g_sta_param_ptr->cipher_suite = WIFI_SECURITY_NONE;
	ssid->wep_key[0][0] = 0;
...
int wpa_config_set_wep(struct wpa_ssid *ssid)
{
	...
	if (!ret) {
		g_sta_param_ptr->cipher_suite = WIFI_SECURITY_WEP;
	} else {
...
struct wpa_config *wpa_config_read(const char *name, struct wpa_config *cfgp)
{
	...
	config->p2p_ssid_postfix = os_strdup("-beken");
	config->num_p2p_pref_chan = 1;
	config->p2p_pref_chan = (struct p2p_channel *)os_malloc(config->num_p2p_pref_chan * sizeof(struct p2p_channel));
	if (config->p2p_pref_chan == NULL) {
		errors++;
		goto error;
	}
	config->p2p_pref_chan[0].op_class = 81;
	config->p2p_pref_chan[0].chan = 13;
#endif
