...
#ifndef CONFIG_WIFI_FAST_CONNECT
...
#if CONFIG_BRIDGE
			struct wpabuf *bridge_ie = wpa_bss_get_bridge_vendor_ie(bss, TYBRIDGE_VENDOR_TYPE);
			if (NULL == bridge_ie) {
				results->ApList[index].is_bridge = 0;
			} else {
				results->ApList[index].is_bridge = 1;
				wpabuf_free(bridge_ie);
			}
#endif
			index++;
		}
...
#if BK_SUPPLICANT
	case WPA_CTRL_CMD_AP_DEAUTH_ALL_STA: {
		struct hostapd_data *hapd;
...
#define CHECK_HAPD()\
	...
	if (msg->cmd == WPA_CTRL_EVENT_CHAN_SWITCH_IND ||
		...
		msg->cmd == WPA_CTRL_EVENT_AP_SCAN_RESULTS)
		CHECK_HAPD();
	else
		CHECK_WPA_S();
