    ...
    while (payload_len >= 2)
    {
            ...
            payload_len >= opt_len)
        {
            ...
            if (rio_data_len >= prefix_len_bytes)
            {
                ...
                LWIP_LOGI("prefix %s lifetime %lu\n", ip6addr_ntoa(&prefix), route.lifetime_seconds);
                if (bk_route_table_add_route_entry(&route) == NULL)
                {
                    LWIP_LOGI("Failed to add route table entry\n");
                }
                else
                {
                    LWIP_LOGI("Added entry to route table\n");
                }
            }
        }
...
static uint8_t icmp6_raw_recv_handler(void * arg, struct raw_pcb * pcb, struct pbuf * p, const ip_addr_t * addr)
{
    ...
    if (p->tot_len != p->len)
    {
        LWIP_LOGI("Ignore segmented ICMP packet\n");
        return 0;
    }
    if (p->tot_len <= sizeof(struct ip6_hdr) + sizeof(struct icmp6_hdr))
    {
        LWIP_LOGI("Ignore invalid ICMP packet\n");
        return 0;
    }
...
int8_t bk_route_hook_init()
{
    ...
    if (lwip_netif == NULL)
    {
        LWIP_LOGI("Invalid network interface\n");
        return -1;
    }
    ...
    for (bk_route_hook_t * iter = s_hooks; iter != NULL; iter++)
    {
        ...
        {
            LWIP_LOGI("Hook already installed on netif, skip...\n");
            return 0;
        }
    }
    ...
    if (hook == NULL)
    {
        LWIP_LOGI("Cannot allocate hook\n");
        return -1;
    }

    if (mld6_joingroup_netif(lwip_netif, ip_2_ip6(&router_group)) != ERR_OK)
    {
        LWIP_LOGI("Failed to join multicast group\n");
        ret = -1;
        goto exit;
    }
