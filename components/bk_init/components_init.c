...
static bool s_components_is_inited = false;
...
{
#if (CONFIG_SOC_BK7236XX) && (!CONFIG_SOC_BK7236N) && (CONFIG_OTP) && (CONFIG_SYS_CPU0)
	uint8_t new_bandgap;
	uint8_t old_bandgap;
	bk_err_t result;
...
#elif ((CONFIG_SOC_BK7236N) || (CONFIG_SOC_BK7239XX)) && (CONFIG_OTP) && (CONFIG_SYS_CPU0)
	uint8_t new_bandgap;
	uint8_t old_bandgap;
	bk_err_t result;
...
int components_early_init(void)
{
	...
	sys_hal_deep_lv_fast_boot_vector_init();
#endif
