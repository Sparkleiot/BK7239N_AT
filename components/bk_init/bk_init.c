...
#endif
#if defined(CONFIG_MQTT_CLIENT_AT_ENABLE) && defined(CONFIG_WIFI_ENABLE)
#include "mqtt_at.h"
#endif
#if defined(CONFIG_HTTP_CLIENT_AT_ENABLE) && defined(CONFIG_WIFI_ENABLE)
#include "http_at.h"
#endif
...
static int app_wifi_init(void)
{
	...
	mqtt_at_cmd_init();
#endif
#if CONFIG_HTTP_CLIENT_AT_ENABLE
	http_at_cmd_init();
#endif
...
extern uint8_t _get_ble_pwr_idx_wrapper(uint8_t channel);
extern void _ble_cal_set_txpwr_wrapper(uint8_t idx);
extern int calibration_init(void);

int bk_cali_customer_init(void)
{
	int ret;

	ret = calibration_init();
	if (ret != BK_OK) {
		BK_LOGW(NULL, "calibration init failed, ret(%x)\r\n", ret);
		return ret;
	}
...
#if !CONFIG_CUSTOMER_INIT_BLE
	tx_pwr_idx = _get_ble_pwr_idx_wrapper(19);
	_ble_cal_set_txpwr_wrapper(tx_pwr_idx);
#endif
...
#if CONFIG_BK_OTA
static bool is_deep_sleep_wakeup(void)
{
	uint32_t reset_reason = bk_misc_get_reset_reason();

	BK_LOGD(TAG, "reset_reason: %x\r\n", reset_reason);

	return (reset_reason == RESET_SOURCE_DEEPPS_GPIO) ||
...
#endif
#if CONFIG_PM_CLIENT && CONFIG_PM_ENABLE
