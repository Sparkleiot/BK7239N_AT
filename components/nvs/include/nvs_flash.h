 ...
 *      - one of the error codes from the underlying flash storage driver
 */
bk_err_t nvs_flash_init_partition(const char *partition_label);
 ...
 *      - one of the error codes from the underlying flash storage driver
 */
bk_err_t nvs_flash_init_partition_ptr(const bk_logic_partition_t *partition);
 ...
 */
bk_err_t nvs_flash_deinit(void);
 ...
 */
bk_err_t nvs_flash_deinit_partition(const char* partition_label);
 ...
 *      - different error in case de-initialization fails (shouldn't happen)
 */
bk_err_t nvs_flash_erase_partition(const char *part_name);
 ...
 *      - one of the error codes from the underlying flash storage driver
 */
bk_err_t nvs_flash_erase_partition_ptr(const bk_logic_partition_t *partition);
 ...
 *      - one of the error codes from the underlying flash storage driver
 */
bk_err_t nvs_flash_secure_init(nvs_sec_cfg_t* cfg);
 ...
 *      - one of the error codes from the underlying flash storage driver
 */
bk_err_t nvs_flash_secure_init_partition(const char *partition_label, nvs_sec_cfg_t* cfg);
...
bk_err_t nvs_flash_generate_keys(const bk_logic_partition_t* partition, nvs_sec_cfg_t* cfg);
...
bk_err_t nvs_flash_read_security_cfg(const bk_logic_partition_t* partition, nvs_sec_cfg_t* cfg);


/**
 * @brief Read NVS security configuration from a partition id.
 *
 * @param[in]  partition use partition id
 *                       Must be valid.
 * @param[out] cfg       Pointer to nvs security configuration structure.
 *                       Pointer must be non-NULL.
 *
...
bk_err_t nvs_flash_read_cfg_by_partId(bk_partition_t partition, nvs_sec_cfg_t* cfg);
