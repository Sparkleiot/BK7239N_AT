...
TEST_CASE("nvs iterators tests", "[nvs]")
{
    ...
    auto entry_count = [](const char *part, const char *name, nvs_type_t type)-> int {
        ...
        bk_err_t res = nvs_entry_find(part, name, type, &it);
    ...
    SECTION("All fields of nvs_entry_info_t structure are correct") {
        it = nullptr;
        bk_err_t res = nvs_entry_find(NVS_DEFAULT_PART_NAME, name_1, NVS_TYPE_I32, &it);
        REQUIRE(res == ESP_OK);
        string key = "value5";
        while (res == ESP_OK) {
    ...
    SECTION("Iterating over multiple pages works correctly") {
        ...
        int entries_found = 0;
        it = nullptr;
        bk_err_t res = nvs_entry_find(NVS_DEFAULT_PART_NAME, name_3, NVS_TYPE_ANY, &it);
        while (res == ESP_OK) {
            entries_found++;
            res = nvs_entry_next(&it);
        }
...
public:
    ...
    bk_err_t doRandomThings(nvs_handle_t handle, TGen gen, size_t &count)
    {
        ...
        auto randomRead = [&](size_t index) -> bk_err_t {
            switch (types[index])
            {
        ...
        auto randomWrite = [&](size_t index) -> bk_err_t {
            switch (types[index])
            {
            case nvs::ItemType::I32: {
    ...
    bk_err_t handleExternalWriteAtIndex(uint8_t index, const void *value, const size_t len )
    {
