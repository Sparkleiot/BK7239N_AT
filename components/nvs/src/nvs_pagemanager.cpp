...
namespace nvs
{
bk_err_t PageManager::load(Partition *partition, uint32_t baseSector, uint32_t sectorCount)
{
...
bk_err_t PageManager::requestNewPage()
{
    if (mFreePageList.empty()) {
...
bk_err_t PageManager::activatePage()
{
    if (mFreePageList.empty()) {
        return ESP_ERR_NVS_NOT_ENOUGH_SPACE;
    }
...
bk_err_t PageManager::fillStats(nvs_stats_t& nvsStats)
{
