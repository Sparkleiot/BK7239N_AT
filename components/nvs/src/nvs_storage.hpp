...
public:
    ...
    bk_err_t init(uint32_t baseSector, uint32_t sectorCount);

    bool isValid() const;

    bk_err_t createOrOpenNamespace(const char* nsName, bool canCreate, uint8_t& nsIndex);

    bk_err_t writeItem(uint8_t nsIndex, ItemType datatype, const char* key, const void* data, size_t dataSize);

    bk_err_t readItem(uint8_t nsIndex, ItemType datatype, const char* key, void* data, size_t dataSize);

    bk_err_t getItemDataSize(uint8_t nsIndex, ItemType datatype, const char* key, size_t& dataSize);

    bk_err_t eraseItem(uint8_t nsIndex, ItemType datatype, const char* key);
    ...
    bk_err_t eraseNamespace(uint8_t nsIndex);
    ...
    bk_err_t writeMultiPageBlob(uint8_t nsIndex, const char* key, const void* data, size_t dataSize, VerOffset chunkStart);

    bk_err_t readMultiPageBlob(uint8_t nsIndex, const char* key, void* data, size_t dataSize);

    bk_err_t cmpMultiPageBlob(uint8_t nsIndex, const char* key, const void* data, size_t dataSize);

    bk_err_t eraseMultiPageBlob(uint8_t nsIndex, const char* key, VerOffset chunkStart = VerOffset::VER_ANY);
    ...
    bk_err_t calcEntriesInNamespace(uint8_t nsIndex, size_t& usedEntries);
...
protected:
    ...
    bk_err_t populateBlobIndices(TBlobIndexList&);

    void eraseOrphanDataBlobs(TBlobIndexList&);

    void fillEntryInfo(Item &item, nvs_entry_info_t &info);

    bk_err_t findItem(uint8_t nsIndex, ItemType datatype, const char* key, Page* &page, Item& item, uint8_t chunkIdx = Page::CHUNK_ANY, VerOffset chunkStart = VerOffset::VER_ANY);
