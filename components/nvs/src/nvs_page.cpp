...
bk_err_t Page::load(Partition *partition, uint32_t sectorNumber)
{
...
bk_err_t Page::writeEntry(const Item& item)
{
    uint32_t phyAddr;
    bk_err_t err = getEntryAddress(mNextFreeEntry, &phyAddr);
    if (err != ESP_OK) {
        return err;
    }
...
bk_err_t Page::writeEntryData(const uint8_t* data, size_t size)
{
    ...
    uint32_t phyAddr;
    bk_err_t rc = getEntryAddress(mNextFreeEntry, &phyAddr);
    if (rc == ESP_OK) {
...
bk_err_t Page::writeItem(uint8_t nsIndex, ItemType datatype, const char* key, const void* data, size_t dataSize, uint8_t chunkIdx)
{
    Item item;
    bk_err_t err;
...
bk_err_t Page::readItem(uint8_t nsIndex, ItemType datatype, const char* key, void* data, size_t dataSize, uint8_t chunkIdx, VerOffset chunkStart)
{
    size_t index = 0;
    Item item;
    ...
    bk_err_t rc = findItem(nsIndex, datatype, key, index, item, chunkIdx, chunkStart);
    if (rc != ESP_OK) {
        return rc;
    }
...
bk_err_t Page::cmpItem(uint8_t nsIndex, ItemType datatype, const char* key, const void* data, size_t dataSize, uint8_t chunkIdx, VerOffset chunkStart)
{
    size_t index = 0;
    Item item;
...
bk_err_t Page::eraseItem(uint8_t nsIndex, ItemType datatype, const char* key, uint8_t chunkIdx, VerOffset chunkStart)
{
    size_t index = 0;
    Item item;
    bk_err_t rc = findItem(nsIndex, datatype, key, index, item, chunkIdx, chunkStart);
...
bk_err_t Page::findItem(uint8_t nsIndex, ItemType datatype, const char* key, uint8_t chunkIdx, VerOffset chunkStart)
{
    size_t index = 0;
    Item item;
...
bk_err_t Page::eraseEntryAndSpan(size_t index)
{
    uint32_t seq_num;
    getSeqNumber(seq_num);

    EntryState state;
    bk_err_t err = mEntryTable.get(index, &state);
    if (err != ESP_OK) {
        return err;
    }
...
bk_err_t Page::updateFirstUsedEntry(size_t index, size_t span)
{
...
bk_err_t Page::copyItems(Page& other)
{
    if (mFirstUsedEntry == INVALID_ENTRY) {
        return ESP_ERR_NVS_NOT_FOUND;
    }
...
bk_err_t Page::alterEntryState(size_t index, EntryState state)
{
    NVS_ASSERT_OR_RETURN(index < ENTRY_COUNT, ESP_FAIL);
    bk_err_t err = mEntryTable.set(index, state);
    if (err != ESP_OK) {
        return err;
    }
...
bk_err_t Page::alterEntryRangeState(size_t begin, size_t end, EntryState state)
{
    NVS_ASSERT_OR_RETURN(end <= ENTRY_COUNT, ESP_FAIL);
...
bk_err_t Page::alterPageState(PageState state)
{
...
bk_err_t Page::readEntry(size_t index, Item& dst) const
{
    uint32_t phyAddr;
    bk_err_t rc = getEntryAddress(index, &phyAddr);
    if (rc != ESP_OK) {
        return rc;
    }
...
bk_err_t Page::findItem(uint8_t nsIndex, ItemType datatype, const char* key, size_t &itemIndex, Item& item, uint8_t chunkIdx, VerOffset chunkStart)
{
...
bk_err_t Page::getSeqNumber(uint32_t& seqNumber) const
{
...
bk_err_t Page::setSeqNumber(uint32_t seqNumber)
{
    if (mState != PageState::UNINITIALIZED) {
        return ESP_ERR_NVS_INVALID_STATE;
    }
...
bk_err_t Page::setVersion(uint8_t ver)
{
    if (mState != PageState::UNINITIALIZED) {
        return ESP_ERR_NVS_INVALID_STATE;
    }
...
bk_err_t Page::calcEntries(nvs_stats_t &nvsStats)
{
