...
bk_err_t NVSPartitionManager::init_partition(const char *partition_label)
{
    ...
    bk_err_t result = partition_lookup::lookup_nvs_partition(partition_label, &p);
...
bk_err_t NVSPartitionManager::init_custom(Partition *partition, uint32_t baseSector, uint32_t sectorCount)
{
    ...
    bk_err_t err = storage->init(baseSector, sectorCount);
    if (new_storage != nullptr) {
        if (err == ESP_OK) {
...
#if CONFIG_NVS_ENCRYPTION
bk_err_t NVSPartitionManager::secure_init_partition(const char *part_name, nvs_sec_cfg_t* cfg)
{
...
bk_err_t NVSPartitionManager::deinit_partition(const char *partition_label)
{
...
bk_err_t NVSPartitionManager::open_handle(const char *part_name,
        const char *ns_name,
        ...
        NVSHandleSimple** handle)
{
    ...
    bk_err_t err = sHandle->createOrOpenNamespace(ns_name, open_mode == NVS_READWRITE, nsIndex);
    if (err != ESP_OK) {
        return err;
    }
...
bk_err_t NVSPartitionManager::close_handle(NVSHandleSimple* handle) {
    for (auto it = nvs_handles.begin(); it != nvs_handles.end(); ++it) {
