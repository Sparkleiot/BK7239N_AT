...
public:
    ...
    bk_err_t load(Partition *partition, uint32_t sectorNumber);

    bk_err_t getSeqNumber(uint32_t& seqNumber) const;

    bk_err_t setSeqNumber(uint32_t seqNumber);

    bk_err_t setVersion(uint8_t version);

    bk_err_t writeItem(uint8_t nsIndex, ItemType datatype, const char* key, const void* data, size_t dataSize, uint8_t chunkIdx = CHUNK_ANY);

    bk_err_t readItem(uint8_t nsIndex, ItemType datatype, const char* key, void* data, size_t dataSize, uint8_t chunkIdx = CHUNK_ANY, VerOffset chunkStart = VerOffset::VER_ANY);

    bk_err_t cmpItem(uint8_t nsIndex, ItemType datatype, const char* key, const void* data, size_t dataSize, uint8_t chunkIdx = CHUNK_ANY, VerOffset chunkStart = VerOffset::VER_ANY);

    bk_err_t eraseItem(uint8_t nsIndex, ItemType datatype, const char* key, uint8_t chunkIdx = CHUNK_ANY, VerOffset chunkStart = VerOffset::VER_ANY);

    bk_err_t findItem(uint8_t nsIndex, ItemType datatype, const char* key, uint8_t chunkIdx = CHUNK_ANY, VerOffset chunkStart = VerOffset::VER_ANY);

    bk_err_t findItem(uint8_t nsIndex, ItemType datatype, const char* key, size_t &itemIndex, Item& item, uint8_t chunkIdx = CHUNK_ANY, VerOffset chunkStart = VerOffset::VER_ANY);

    template<typename T>
    bk_err_t writeItem(uint8_t nsIndex, const char* key, const T& value)
    {
        return writeItem(nsIndex, itemTypeOf(value), key, &value, sizeof(value));
    }

    template<typename T>
    bk_err_t readItem(uint8_t nsIndex, const char* key, T& value)
    {
        return readItem(nsIndex, itemTypeOf(value), key, &value, sizeof(value));
    }

    template<typename T>
    bk_err_t cmpItem(uint8_t nsIndex, const char* key, const T& value)
    {
        return cmpItem(nsIndex, itemTypeOf(value), key, &value, sizeof(value));
    }

    template<typename T>
    bk_err_t eraseItem(uint8_t nsIndex, const char* key)
    {
    ...
    bk_err_t markFreeing();

    bk_err_t copyItems(Page& other);

    bk_err_t erase();

    void debugDump() const;

    bk_err_t calcEntries(nvs_stats_t &nvsStats);
...
protected:
    ...
    bk_err_t mLoadEntryTable();

    bk_err_t initialize();

    bk_err_t alterEntryState(size_t index, EntryState state);

    bk_err_t alterEntryRangeState(size_t begin, size_t end, EntryState state);

    bk_err_t alterPageState(PageState state);

    bk_err_t readEntry(size_t index, Item& dst) const;

    bk_err_t writeEntry(const Item& item);

    bk_err_t writeEntryData(const uint8_t* data, size_t size);

    bk_err_t eraseEntryAndSpan(size_t index);

    bk_err_t updateFirstUsedEntry(size_t index, size_t span);
    ...
    bk_err_t getEntryAddress(size_t entry, uint32_t *address) const
    {
