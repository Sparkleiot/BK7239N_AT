                   ...
                   struct boot_status *bs)
{
    ...
    if (!boot_is_header_valid(hdr, fap) || FIH_NOT_EQ(fih_rc, FIH_SUCCESS)) {
        ...
        bk_boot_write_ota_fail(BOOT_FAIL_STAGE_APP_HDR);
#endif
#if !defined(__BOOTSIM__)
        ...
        for (slot = 0; slot < BOOT_NUM_SLOTS; slot++) {
            ...
            if (boot_is_header_valid(hdr, BOOT_IMG_AREA(state, slot))) {
                ...
                if (slot == 0) {
                    ...
                    bk_boot_write_ota_fail(BOOT_FAIL_STAGE_OTA_HDR);
                }
#endif
...
boot_select_or_erase(struct boot_loader_state *state)
{
    ...
    if (active_swap_state->magic != BOOT_MAGIC_GOOD ||
         ...
         * Secondary slot is never erased; only primary slot may be erased.
         */
        if (active_slot == BOOT_PRIMARY_SLOT) {
            BOOT_LOG_DBG("Erasing faulty image in the primary slot.");
            rc = flash_area_erase(fap, 0, flash_area_get_size(fap));
            assert(rc == 0);
        } else {
            BOOT_LOG_DBG("Skipping erase of faulty image in the secondary slot.");
        }
