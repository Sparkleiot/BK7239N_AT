...
void cli_wifi_set_sleep_mode_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
       ...
       sleep_mode = (uint8_t)os_strtoul(argv[1], NULL, 10);
       ret = bk_wifi_send_sleep_mode_req(sleep_mode);

       if (!ret) {
               CLI_LOGI("set sleep_mode ok\n");
               msg = WIFI_CMD_RSP_SUCCEED;
               os_memcpy(pcWriteBuffer, msg, os_strlen(msg));
               return;
       }
       else {
               CLI_LOGI("set sleep_mode failed\n");
               goto error;
       }
					   ...
					   int event_id, void *event_data)
{
	...
	case EVENT_NETIF_GOT_IP6:
		...
		CLI_LOGI("%s got ip%d:%s\n", got_ip->netif_if == NETIF_IF_STA ? "BK STA" : "unknown netif",
			event_id== EVENT_NETIF_GOT_IP4 ? 4 : 6, got_ip->ip);
		break;
	default:
					  ...
					  int event_id, void *event_data)
{
	...
	case EVENT_WIFI_DTIM_RECEIVED:
		break;

	case EVENT_WIFI_AWARE_WAKE_INTERVAL_START:
		break;
#endif
...
void cli_rlk_cfg_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if(os_strcmp(argv[1], "rate") == 0) {
		...
		rlk_cfg = os_strtoul(argv[2], NULL, 10) & 0xFFFF;
		bk_rlk_set_format(rlk_cfg);
	}
	else if(os_strcmp(argv[1], "bw") == 0) {
		rlk_cfg = os_strtoul(argv[2], NULL, 10) & 0xFFFF;
		bk_rlk_set_band_width(rlk_cfg);
	}
	else if(os_strcmp(argv[1], "gi") == 0) {
		rlk_cfg = os_strtoul(argv[2], NULL, 10) & 0xFFFF;
		bk_rlk_set_gi_type(rlk_cfg);
	}
	else if(os_strcmp(argv[1], "mcs") == 0) {
		rlk_cfg = os_strtoul(argv[2], NULL, 10) & 0xFFFF;
		bk_rlk_set_ht_mcs(rlk_cfg);
	}
	else if(os_strcmp(argv[1], "channel") == 0) {
		prim_channel = os_strtoul(argv[2], NULL, 10) & 0xFFFF;
		second_channel = os_strtoul(argv[3], NULL, 10) & 0xFFFF;
		bk_rlk_set_channel(prim_channel, second_channel);
	}
	else {
...
void cli_wifi_capa_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	} else if (os_strcmp(argv[1], "tx_use_max_bw") == 0) {
		capa_id = WIFI_CAPA_ID_TX_USE_MAX_BW;
...
void cli_wifi_csi_cmd(char * pcWriteBuffer, int xWriteBufferLen, int argc, char * * argv)
{
	extern bk_err_t bk_wifi_csi_active_mode_req(wifi_csi_active_mode_config_t *config);
	wifi_csi_active_mode_config_t active_mode_config = {0};
	active_mode_config.enable = true;
	active_mode_config.interval = 200;
	extern bk_err_t bk_wifi_csi_receive_mode_req(wifi_csi_receive_mode_config_t *config);
	wifi_csi_receive_mode_config_t receive_mode_config = {0};
	receive_mode_config.enable = true;
	uint8_t base_mac[BK_MAC_ADDR_LEN] = {0};

	receive_mode_config.filter_config.proto_type_bmp = 0xF;// all format
	receive_mode_config.filter_config.cbw_bmp = 0x3; // 20M/40M
	receive_mode_config.filter_config.filter_mac_addr_type = 0; // all mac
	receive_mode_config.filter_config.filter_mgmt_frame_bmp = 0x80000000;
	receive_mode_config.filter_config.filter_ctrl_frame_bmp = 0;
	receive_mode_config.filter_config.filter_data_frame_bmp = 0x80000000;

	if(os_strcmp(argv[1], "start") == 0)
	{
		if (argc > 2)
		{
			active_mode_config.interval = os_strtoul(argv[2], NULL, 10);
		}
		CLI_LOGI("csi active mode interval %d\r\n",active_mode_config.interval);
		bk_wifi_csi_active_mode_req(&active_mode_config);
	}
	else if(os_strcmp(argv[1], "stop") == 0)
	{
		active_mode_config.enable = false;
		bk_wifi_csi_active_mode_req(&active_mode_config);
	}
	else if(os_strcmp(argv[1], "mode_config") == 0)
	{
		if (argc > 2)
		{
			receive_mode_config.filter_config.proto_type_bmp = os_strtoul(argv[2], NULL, 16);
			if(argc > 3)
			{
				receive_mode_config.filter_config.cbw_bmp = os_strtoul(argv[3], NULL, 16);

				if(argc > 4)
				{
					receive_mode_config.filter_config.filter_mac_addr_type = os_strtoul(argv[4], NULL, 10);
					uint8_t argc_offset = 5;

					if(receive_mode_config.filter_config.filter_mac_addr_type == 0)
					{
						CLI_LOGI("err param 4, filter_mac_addr_type %d\r\n",receive_mode_config.filter_config.filter_mac_addr_type);
					}
					else if ((receive_mode_config.filter_config.filter_mac_addr_type == 1)||
						(receive_mode_config.filter_config.filter_mac_addr_type == 3) ||
						(receive_mode_config.filter_config.filter_mac_addr_type == 4))
					{
						if(argc > argc_offset)
						{
							receive_mode_config.filter_config.filter_src_mac_num = os_strtoul(argv[argc_offset], NULL, 10);
							argc_offset++;
						}
						else
						{
							CLI_LOGI("err param 1\r\n");
							return;
						}

						uint8_t mac_num = receive_mode_config.filter_config.filter_src_mac_num;
						CLI_LOGI("mac_num %d\r\n", mac_num);
						CLI_LOGI("argc_offset %d\r\n", argc_offset);
						CLI_LOGI("argc %d\r\n", argc);
						if(argc > (argc_offset+mac_num-1))
						{
							for(uint8_t i=0;i<mac_num;i++)
							{
								if(cli_wifi_csi_read_addr(argv[argc_offset],base_mac))
								{
									os_memcpy((receive_mode_config.filter_config.filter_src_mac+i*6),base_mac,6);
									argc_offset++;
								}
								else
								{
									CLI_LOGI("err param 2\r\n");
									return;
								}
							}
						}
						...
						{
							CLI_LOGI("err param 3 , argc %d, argc_offset %d, mac_num %d\r\n", argc, argc_offset, mac_num);
							return;
						}
					}
					if((receive_mode_config.filter_config.filter_mac_addr_type == 1)||
						(receive_mode_config.filter_config.filter_mac_addr_type == 2) ||
						(receive_mode_config.filter_config.filter_mac_addr_type == 4))
					{
						if(argc > argc_offset)
						{
							receive_mode_config.filter_config.filter_dst_mac_num = os_strtoul(argv[argc_offset], NULL, 10);
							argc_offset++;
						}
						else
						{
							CLI_LOGI("err param 4\r\n");
							return;
						}

						uint8_t mac_num = receive_mode_config.filter_config.filter_dst_mac_num;
						if(argc > (argc_offset+mac_num-1))
						{
							for(uint8_t i=0;i<mac_num;i++)
							{
								if(cli_wifi_csi_read_addr(argv[argc_offset],base_mac))
								{
									os_memcpy((receive_mode_config.filter_config.filter_dst_mac+i*6),base_mac,6);
									argc_offset++;
								}
								else
								{
									CLI_LOGI("err param 5\r\n");
									return;
								}
							}
						}
						...
						{
							CLI_LOGI("err param 6 , argc %d, argc_offset %d, mac_num %d\r\n", argc, argc_offset, mac_num);
							return;
						}
					}
				}
		...
		bk_wifi_csi_receive_mode_req(&receive_mode_config);
	}
	else if(os_strcmp(argv[1], "mode_disable") == 0)
	{
		receive_mode_config.enable = false;
		bk_wifi_csi_receive_mode_req(&receive_mode_config);
	}
}
#endif
...
#endif
#if CONFIG_WIFI_SLEEP_MODE_SUPPORT
	{"sleep_mode", "set sleep_mode", cli_wifi_set_sleep_mode_cmd},
#endif
...
#endif
#if CONFIG_WIFI_CSI_EN
	{"csi", "csi {start|stop}", cli_wifi_csi_cmd},
#endif
};
