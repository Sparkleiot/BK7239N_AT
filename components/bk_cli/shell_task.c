...
#if defined(CONFIG_AT) && defined(CONFIG_SYS_CPU0) && (CONFIG_UART_PRINT_PORT == AT_UART_PORT_CFG)
#define SHELL_PROMPT_OUTPUT_ENABLE 0
#else
#define SHELL_PROMPT_OUTPUT_ENABLE 1
#endif

#if SHELL_PROMPT_OUTPUT_ENABLE
...
static bool shell_log_dev_ready(void)
{
	return log_dev != NULL
		&& log_dev->dev_drv != NULL
		&& log_dev->dev_drv->io_ctrl != NULL;
}
...
__IRAM_SEC static void shell_power_save_enter(void)
{
	if(log_tx_init_ok)
	{
...
__IRAM_SEC static void shell_power_save_exit(void)
{
	if(cmd_dev->dev_type == SHELL_DEV_UART)
	{
		u8   uart_port = UART_ID_MAX;
...
static void shell_print_startup_banner(void)
{
	static const u8 startup_banner[] = "\r\n\r\nhttps://github.com/Sparkleiot\r\n\r\nready\r\n";

	shell_log_raw_data(startup_banner, sizeof(startup_banner) - 1);
}
...
static bool shell_should_drop_log(const char *format)
{
	static const char rfconfig_log_prefix[] = "rwnx_cal_set_rfconfig(";

	if (format == NULL) {
		return false;
	}

	if (strncmp(format, rfconfig_log_prefix, sizeof(rfconfig_log_prefix) - 1) == 0) {
		return true;
	}
...
void shell_log_out_port(int block_mode, int level, char *prefix, const char *format, va_list ap)
{
	...
	if (shell_should_drop_log(format))
	{
		return;
	}
