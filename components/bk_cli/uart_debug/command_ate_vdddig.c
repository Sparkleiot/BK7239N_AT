...
int sctrl_set_bandgap_to_efuse(const unsigned char *content, int cnt, UINT8 *tx_buffer)
{
    ...
    os_memset(bandgap, 0, sizeof(bandgap));
    bandgap[0] = content[2];
    if (bandgap[0] > BANDGAP_BGCAL_MANUAL_MASK)
    {
        tx_buffer[1] = 0xDD;
        uart_send_bytes_for_ate(tx_buffer, 2);
        return -1;
    ...
    {
        /* success */
        sys_hal_set_bgcalm(bandgap[0]);
        tx_buffer[1] = 0x33;
        bk_otp_ahb_read(OTP_VDDDIG_BANDGAP, tx_buffer + 2, 1);
...
int sctrl_set_dia_to_efuse(const unsigned char *content, int cnt, UINT8 *tx_buffer)
{
    ...
    if (cnt < 3)
    {
        ...
        {
            /* success */
            rwnx_cal_set_reg_mod_pa(0, dia[0]);
            tx_buffer[0] = 0x0E;
            tx_buffer[1] = dia[0];
            uart_send_bytes_for_ate(tx_buffer, 2);
            return 0;
    ...
    if (bk_otp_ahb_update(OTP_DIA, dia, sizeof(dia)) == 0)
    {
        /* success */
        tx_buffer[1] = 0x33;
        bk_otp_ahb_read(OTP_DIA, tx_buffer + 2, 1);
        rwnx_cal_dia_stop();
        uart_send_bytes_for_ate(tx_buffer, 3);
    }
    else
