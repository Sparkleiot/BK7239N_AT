...
#endif
#if CONFIG_NORMAL_SLEEP_ULTRA_LOW_POWER
extern bk_err_t sys_hal_normal_sleep_mode_ctrl(pm_normal_sleep_mode_e normal_sleep);
static void cli_pm_ns(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	switch(normal_sleep_flag)
	{
		...
		{
			sys_hal_normal_sleep_mode_ctrl(normal_sleep_flag);
			bk_bluetooth_init();
		}
		break;
...
static void cli_pm_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	else if(pm_wake_source == PM_WAKEUP_SOURCE_INT_GPIO)
	{
		...
		else if((pm_sleep_mode == PM_MODE_LOW_VOLTAGE)||(pm_sleep_mode == PM_MODE_DEEP_SLEEP))
		{
...
#if CONFIG_ANA_RTC
			os_printf("gpio 13 output analog rosc 32k\r\n");
#else
			reg = REG_READ(SYSTEM_BASE_ADDR + 0x45 * 4);     // 0x44010114
			reg |= BIT(12);
			REG_WRITE((SYSTEM_BASE_ADDR + 0x45 * 4), reg);   // 0x44010114
