...
extern void cli_psram_xip_stress_test(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv);
#endif
...
#if CONFIG_PSRAM_CALIBRATE
	else if (os_strcmp(argv[1], "calibrate") == 0)
	{
		bk_psram_init();
		psram_7239n_calibrate_test();
		msg = CLI_CMD_RSP_SUCCEED;
	}
#endif
	else
...
static void cli_psram_cmd_handle_ext(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	else if (os_strcmp(argv[1], "voltage") == 0)
	{
		if (os_strcmp(argv[2], "3_0V") == 0) {
			bk_psram_set_voltage(PSRAM_OUT_3_0V);
		} else if (os_strcmp(argv[2], "1_95V") == 0) {
			bk_psram_set_voltage(PSRAM_OUT_1_95V);
		} else if (os_strcmp(argv[2], "1_90V") == 0) {
			bk_psram_set_voltage(PSRAM_OUT_1_90V);
		} else if (os_strcmp(argv[2], "1_85V") == 0) {
			bk_psram_set_voltage(PSRAM_OUT_1_85V);
		} else if (os_strcmp(argv[2], "1_80V") == 0) {
			bk_psram_set_voltage(PSRAM_OUT_1_80V);
		}
	...
	else if (os_strcmp(argv[1], "word") == 0)
	{
		...
		else
		{
			...
			for (i = 0; i < length; i += 4)
			{
				...
				if (value != ((i << 24) + (i << 16) + (i << 8) + i + 0x11223344))
				{
					err_flag = 1;
					CLI_LOGI("index:%d, value:%x\r\n", i, value);
					break;
				}
			}
		}
...
static beken_thread_t s_psram_rw_task_hdl = NULL;
static beken_thread_t s_psram_rw_task_hd2 = NULL;

static void psram_rw_task1(void)
{
	...
	bk_psram_init();
	while (1) {
		CLI_LOGI("psram_rw_task1 is running.\r\n");
		for (i = 0; i < length; i += 4) {
			*(((volatile uint32_t *)(addr + i))) = 0x55AA55AA;
		}
	...
	rtos_delete_thread(NULL);
	s_psram_rw_task_hdl = NULL;
}

static void psram_rw_task2(void)
{
	uint32_t addr = SOC_PSRAM_DATA_BASE + 0x400000;
	uint32_t length = 0x400000;
	uint32_t i = 0;
	uint32_t value = 0;
	...
	bk_psram_init();
	while (1) {
		CLI_LOGI("psram_rw_task2 is running.\r\n");
		for (i = 0; i < length; i += 4) {
			*(((volatile uint32_t *)(addr + i))) = 0xAA55AA55;
		}
		...
		for (i = 0; i < length; i += 4) {
			...
			if (value != 0xAA55AA55)
			{
				CLI_LOGI("psram_rw_task2 index:%d, value:%x\r\n", i, value);
				break;
			}
		}
	}
	...
	rtos_delete_thread(NULL);
	s_psram_rw_task_hd2 = NULL;
}

static void cli_psram_multitask_rw_handle(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	bk_err_t ret = BK_OK;

	if (!s_psram_rw_task_hdl)
	{
		ret = rtos_create_thread(&s_psram_rw_task_hdl,
								 4,
								 "psram_rw_task1",
								 (beken_thread_function_t)psram_rw_task1,
								 1024,
								 (beken_thread_arg_t)NULL);
		if (ret != BK_OK)
		{
			s_psram_rw_task_hdl = NULL;
			CLI_LOGE("Error: Failed to create psram rw test task1: %d\r\n", ret);
			return;
		}
	}

	if (!s_psram_rw_task_hd2)
	{
		ret = rtos_create_thread(&s_psram_rw_task_hd2,
								 4,
								 "psram_rw_task2",
								 (beken_thread_function_t)psram_rw_task2,
								 1024,
								 (beken_thread_arg_t)NULL);
		if (ret != BK_OK)
		{
			s_psram_rw_task_hd2 = NULL;
			CLI_LOGE("Error: Failed to create psram rw test task2: %d\r\n", ret);
			return;
		}
	}
}
...
{
	write_data(SOC_PSRAM_REG_BASE + 0x5 * 4, value);
}
...
static int psram_7239n_calibration(uint32_t start_addr, uint32_t test_len)
{
	...
	psram_dly_drv *psram_param = (psram_dly_drv *)os_zalloc(512 * sizeof(psram_dly_drv));
	if (psram_param == NULL) {
		os_printf("psram_param malloc failed\r\n");
		return 1;
	}
	...
	{
		for(clks_iodrv = 0; clks_iodrv <= 3; clks_iodrv ++)
		{
			for(dats_iodrv = 0; dats_iodrv <= 3; dats_iodrv ++)
			{
				psram_7239_set_reg5_value((((uint32_t)ds0_delay)<<0) + (((uint32_t)ds1_delay)<<3) + (((uint32_t)clks_iodrv)<<6) + (((uint32_t)dats_iodrv)<<9) + (0 << 12));
	...
	for (k = param_ptr_min; k <= param_ptr_max; k++) {
		printf("ds0 calib success[%u]: ds0_delay=%u, clks_iodrv=%u, dats_iodrv=%u\r\n",
				k,
				psram_param[k].ds0_delay_t,
				psram_param[k].clks_iodrv_t,
				psram_param[k].dats_iodrv_t);
	}
	...
	for (k = param_ptr_min; k <= param_ptr_max; k++) {
		printf("dats calib success[%u]: ds0_delay=%u, clks_iodrv=%u, dats_iodrv=%u\r\n",
				k,
				psram_param[k].ds0_delay_t,
				psram_param[k].clks_iodrv_t,
				psram_param[k].dats_iodrv_t);
...
static void psram_7239n_calibrate_test(void)
{
	...
	ret = psram_7239n_calibration(SOC_PSRAM_DATA_BASE, SOC_PSRAM_8M_SIZE);
	if (ret == BK_OK) {
		os_printf("psram calibrate success\r\n");
	} else {
		os_printf("psram calibrate failed\r\n");
	}
}
#endif
...
static const struct cli_command s_psram_commands[] = {
	...
	{"psram_multitask_rw", "multiple task read and write", cli_psram_multitask_rw_handle},
#if (CONFIG_PSRAM_XIP_STRESS_TEST)
	{"psram_xip_test", "start [duration_sec]|stop|status", cli_psram_xip_stress_test},
#endif
};
