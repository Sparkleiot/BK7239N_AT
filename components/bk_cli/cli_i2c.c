...
static void i2c_fill_seq_buf(uint8_t *buf, uint32_t len, uint8_t start_val)
{
    for (uint32_t i = 0; i < len; i++) {
        buf[i] = (start_val + i) & 0xFF;
    }
}
...
{
    CLI_LOGI("Usage:\r\n");
    CLI_LOGI("  i2c_driver init|deinit\r\n");
    CLI_LOGI("  i2c {id} init [baud] [src_clk]\r\n");
    CLI_LOGI("  i2c {id} deinit\r\n");
    CLI_LOGI("  i2c {id} memory_write {len}\r\n");
    CLI_LOGI("  i2c {id} memory_read {len}\r\n");
    CLI_LOGI("  i2c {id} master_write {len}\r\n");
    CLI_LOGI("  i2c {id} master_read {len}\r\n");
    CLI_LOGI("  i2c {id} master_write_noaddr {len}\r\n");
    CLI_LOGI("  i2c {id} master_read_noaddr {len}\r\n");
    CLI_LOGI("  i2c {id} slave_write {len}\r\n");
    CLI_LOGI("  i2c {id} slave_read {len}\r\n");
    CLI_LOGI("Examples:\r\n");
    CLI_LOGI("  i2c_driver init\r\n");
    CLI_LOGI("  i2c 0 init 400000 0    # init i2c0, 400kHz, default src_clk\r\n");
    CLI_LOGI("  i2c 0 master_write 16  # write 16 bytes to slave (addr preset)\r\n");
    CLI_LOGI("  i2c 0 master_read 16   # read 16 bytes\r\n");
    CLI_LOGI("  i2c 0 master_write_noaddr 10 # write first byte as dev_addr\r\n");
    CLI_LOGI("  i2c 0 master_read_noaddr 10  # read, first byte is dev_addr\r\n");
    CLI_LOGI("  i2c 0 slave_write 8    # prepare 8-byte slave TX buffer\r\n");
    CLI_LOGI("  i2c 0 slave_read 8     # prepare 8-byte slave RX buffer\r\n");
}
...
static void cli_i2c_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    if (os_strcmp(argv[2], "init") == 0) {
    ...
    } else if (os_strcmp(argv[2], "master_write_noaddr") == 0) {
        uint8_t data_buf[10] = {0};
        i2c_fill_seq_buf(data_buf, 10, 0);
        BK_LOG_ON_ERR(bk_i2c_master_write_noaddr(i2c_id, data_buf, 10, I2C_WRITE_WAIT_MAX_MS));
        for (uint32_t i = 0; i < 10; i++) {
            CLI_LOGI("cli_test i2c_master write_noaddr 0x%x,\n", data_buf[i]);
        }
    ...
    } else if (os_strcmp(argv[2], "master_read_noaddr") == 0) {
        uint8_t data_buf[10] = {0};
        BK_LOG_ON_ERR(bk_i2c_master_read_noaddr(i2c_id, data_buf, 10, I2C_READ_WAIT_MAX_MS));
        for (uint32_t i = 0; i < 10; i++) {
            CLI_LOGI("cli_test i2c_master read_noaddr 0x%x,\n", data_buf[i]);
        }
        ...
        i2c_fill_seq_buf(data_buf, buf_len, 1);
        ...
        if (os_strcmp(argv[3], "xtal") == 0){
            sys_drv_i2c_select_clock(i2c_id, I2C_SCLK_XTAL_40M);
            CLI_LOGI("i2c(%d) set clk_source xtal\r\n", i2c_id);
        } else if (os_strcmp(argv[3], "80m") == 0){
            sys_drv_i2c_select_clock(i2c_id, I2C_SCLK_80M);
            CLI_LOGI("i2c(%d) set clk_source 80m\r\n", i2c_id);
        } else {
            CLI_LOGI("i2c config set failed,clock_source only support xtal/80m\r\n");
            return;
        }
        CLI_LOGI("i2c(%d) config clk_source succeed\r\n", i2c_id);
    } else if (os_strcmp(argv[2], "no_ack_mode") == 0) {
        CLI_RET_ON_INVALID_ARGC(argc, 4);
        if (os_strcmp(argv[3], "enable") == 0){
            BK_LOG_ON_ERR(bk_i2c_set_no_ack_mode(i2c_id, true));
            CLI_LOGI("i2c(%d) set no_ack_mode enable\r\n", i2c_id);
        } else if (os_strcmp(argv[3], "disable") == 0){
            BK_LOG_ON_ERR(bk_i2c_set_no_ack_mode(i2c_id, false));
            CLI_LOGI("i2c(%d) set no_ack_mode disable\r\n", i2c_id);
        } else {
            CLI_LOGI("i2c(%d) set no_ack_mode failed, only support enable/disable\r\n", i2c_id);
            return;
        }
        CLI_LOGI("i2c(%d) config no_ack_mode succeed\r\n", i2c_id);
    } else {
        cli_i2c_help();
        return;
    }
...
static const struct cli_command s_i2c_commands[] = {
    ...
    {"i2c", "i2c {id} {init|deinit|memory_write|memory_read|master_write|master_read|master_write_noaddr|master_read_noaddr|slave_write|slave_read|clk_select|no_ack_mode}", cli_i2c_cmd},
};
