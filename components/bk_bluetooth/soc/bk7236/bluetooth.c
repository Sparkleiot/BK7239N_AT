...
static double rc32k_freq_old = 0;
static double rc32k_freq_diff = 0;
...
static double bk_driver_get_rc32k_freq()
{
    ...
    else
    {
        ...
        {
#if CONFIG_CKMN
            double rc32k_freq = bk_rosc_32k_get_freq();

            rc32k_freq_diff = (rc32k_freq_old - rc32k_freq)>0? (rc32k_freq_old - rc32k_freq) : (rc32k_freq - rc32k_freq_old);

            if ((rc32k_freq_diff >= 2500) && (rc32k_freq_old != 0))
            {
                return rc32k_freq_old;
            }
            else
            {
                rc32k_freq_old = rc32k_freq;
                return rc32k_freq;
            }
#else
            return BLUETOOTH_CLK_32K;
...
#if CONFIG_SOC_BK7236N
static void ble_enter_dut_set_vanaldosel(uint8_t value)
{
    uint32_t analog_val = sys_hal_analog_get(ANALOG_REG8);
    sys_hal_analog_set(ANALOG_REG8, (analog_val | (1U << 9))); //spi_latch1v enable

    analog_val = sys_hal_analog_get(ANALOG_REG7);
    sys_hal_analog_set(ANALOG_REG7, ((analog_val & (~0x3C0)) | (value << 6))); //vanaldosel set value

    analog_val = sys_hal_analog_get(ANALOG_REG8);
    sys_hal_analog_set(ANALOG_REG8, (analog_val & ~(1U << 9)));//spi_latch1v disable
}

static void ble_enter_dut_set_xtalh_ctune_wrapper(uint8_t value)
{
    uint32_t analog_val = sys_hal_analog_get(ANALOG_REG2);

    analog_val &= ~(0xFF << 0);
    analog_val |= (value << 0);
    sys_hal_analog_set(ANALOG_REG2, analog_val);
}

static uint8_t ble_enter_dut_get_xtalh_ctune_wrapper(void)
{
    uint32_t analog_val = sys_hal_analog_get(ANALOG_REG2);

    return (analog_val >> 0) & 0xFF;
}
#endif
...
static void ble_enter_dut()
{
    ...
    analog_val = sys_hal_analog_get(ANALOG_REG3);
    sys_hal_analog_set(ANALOG_REG3, (analog_val & ~(1U << 9))); //ck_sel select high hystersis digital buffer

    analog_val = sys_hal_analog_get(ANALOG_REG7);
    sys_hal_analog_set(ANALOG_REG7, ((analog_val & (~0x3C0)) | (0x7 << 6))); //vanaldosel set 3
...
static void ble_exit_dut()
{
    ...
    analog_val = sys_hal_analog_get(ANALOG_REG3);
    sys_hal_analog_set(ANALOG_REG3, (analog_val | (1U << 9))); //ck_sel select high gain digital buffer

    analog_val = sys_hal_analog_get(ANALOG_REG7);
    sys_hal_analog_set(ANALOG_REG7, ((analog_val & (~0x3C0)) | (0xF << 6))); //vanaldosel set F
...
static void coex_bt_request_wrapper(uint32_t event,uint8_t mode,uint32_t duration)
{
    #if (CONFIG_COEX)
    softcoex_bt_request(event,mode,duration);
    #endif
}

static void coex_bt_release_wrapper(uint32_t event)
{
    #if (CONFIG_COEX)
    softcoex_bt_release(event);
    #endif
}
...
static struct bt_osi_funcs_t bt_osi_funcs =
{
    ...
    #if CONFIG_SOC_BK7236N
    ._ble_enter_dut_set_vanaldosel = ble_enter_dut_set_vanaldosel,
    ._ble_enter_dut_set_xtalh_ctune = ble_enter_dut_set_xtalh_ctune_wrapper,
    ._ble_enter_dut_get_xtalh_ctune = ble_enter_dut_get_xtalh_ctune_wrapper,
    #endif
    ...
    ._bk_feature_dsss_only_enable = bk_feature_wifi_dsss_only,
