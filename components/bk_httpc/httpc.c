...
static int httpc_conn_read(struct httpc_conn *conn, uint8_t *buf, size_t buf_len)
{
...
static int httpc_conn_write(struct httpc_conn *conn, uint8_t *buf, size_t buf_len)
{
	if ((conn == NULL) || (buf == NULL)) {
		return -1;
	}
...
int httpc_conn_connect(struct httpc_conn *conn, char *host, uint16_t port, uint32_t timeout)
{
	...
	ret = httpc_tls_hostname_to_fd(host, strlen(host), port, &address, &conn->sock);
	...
	if (conn->secure == HTTPC_SECURE_TLS) {
		if (conn->tls == NULL) {
			BK_LOGE(TAG, "tls context is null\r\n");
			return -1;
		}
...
int httpc_request_write_header_finish(struct httpc_conn *conn) {
    snprintf((char *)conn->request_header + strlen((char *)conn->request_header), SEND_HEADER_SIZE - strlen((char *)conn->request_header),
             "\r\n");
    int bytes_written = httpc_conn_write(conn, conn->request_header, strlen((char *)conn->request_header));
    if (bytes_written < 0) {
        BK_LOGE(TAG,"%s: create failed, no memory!\r\n",__func__);
...
int httpc_request_write_data(struct httpc_conn *conn, uint8_t *data, size_t data_len)
{
    ...
    do
    {
        bytes_write = httpc_conn_write(conn, (void *)((char *)data + total_write), left);
        if (bytes_write <= 0)
        {
...
static int httpc_read_line(struct httpc_conn *conn, char *buffer, int size)
{
    ...
    while (count < size)
    {
        rc = httpc_conn_read(conn, (unsigned char *) &ch, 1);
#if (HTTPC_USE_TLS == HTTPC_TLS_MBEDTLS)
        if (conn->tls && (rc == MBEDTLS_ERR_SSL_WANT_READ || rc == MBEDTLS_ERR_SSL_WANT_WRITE))
        {
            continue;
        }
#endif
...
int httpc_response_read_data(struct httpc_conn *conn, uint8_t *data, size_t data_len)
{
    ...
    do
    {
        ...
        bytes_read = httpc_conn_read(conn, (void *)((char *)data + total_read), left);
        if (bytes_read <= 0)
        {
