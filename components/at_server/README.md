...
| 2     | AT+CIPSTOP  | AT+CIPSTOP=<id>                                             |
| 3     | AT+CIPSEND  | AT+CIPSEND=<id>,<data>                                      |
| 4     | AT+CIPMODE  | AT+CIPMODE? / AT+CIPMODE=<mode>                             |
|       |             |                                                             |
|       |             |                                                             |

当前版本实现说明（MVP）：

1. 仅支持 TCP 客户端连接（`<type>` 仅支持 `TCP`/`tcp`）。
2. 仅支持单连接，`<id>` 固定为 `0`。
3. `AT+CIPSTART` 成功返回 `CMDRSP:OK`，失败返回 `CMDRSP:ERROR`。
4. `AT+CIPSTOP` 仅在当前已有连接时返回 `CMDRSP:OK`，否则返回 `CMDRSP:ERROR`。
5. 需要开启 `NETIF_AT_ENABLE`（默认开启）以注册 `AT+CIPSTART`/`AT+CIPSTOP`。
6. `AT+CIPSEND` 支持两种模式：
    - 文本模式：`AT+CIPSEND=<id>,<data>`
    - 长度模式：`AT+CIPSEND=<id>,<len>`，设备返回 `>` 后继续发送 `<len>` 字节数据。
7. `AT+CIPMODE=1` 时，非 AT 前缀的串口输入会直接透传到 TCP（简单透传模式）。
