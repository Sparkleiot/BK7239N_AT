...
__attribute__((weak)) int atsvr_transparent_send(const char *buf, int len)
{
	return 0;
}

__attribute__((weak)) int atsvr_pending_data_input(const char *buf, int len)
{
	return 0;
}
...
int atsvr_msg_get_input(char *inbuf, int in_buf_size, char * outbuf, int out_buf_size)
{
	...
	if (atsvr_pending_data_input(inbuf, in_buf_size) == 1) {
		return 0;
	}
	int bp = 0;
#if CONFIG_AT_DATA_MODE// at socket
...
cmd_mode:
	...
	while (true)
    {
		if(bp == 0) 
		{
			if(inbuf[0] != 'A') 
			{
				if (atsvr_transparent_send(inbuf, in_buf_size) == 1) {
					break;
				}
