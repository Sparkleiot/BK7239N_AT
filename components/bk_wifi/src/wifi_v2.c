...
extern bool bk_microwave_policy_en;
/**
...
uint32_t bk_sta_cipher_is_open(void)
{
	...
	return (WIFI_SECURITY_NONE == g_sta_param_ptr->cipher_suite);
}
...
uint8_t bk_wlan_ap_get_default_channel(void)
{
	...
	/* If ACS is enabled, let hostpad choose best channel instead of DEFAULT_CHANNEL_AP */
#ifndef CONFIG_ACS_SUPPORT
	if (0 == channel) {
...
#ifdef CONFIG_WIFI_WIRELESS_MODE_SUPPORT
		if (!g_sta_param_ptr->wireless_mode) {
			g_sta_param_ptr->wireless_mode = WIFI_WIRELESS_MODE_BGN;
		}
#endif
...
{
	wlan_ap_sta_deauth_t *req = (wlan_ap_sta_deauth_t *)os_malloc(sizeof(wlan_ap_sta_deauth_t));
	if (req == NULL) {
		WPA_LOGE("wlan_ap_sta_deauth: malloc failed\n");
		return BK_FAIL;
	}

	os_memset(req, 0, sizeof(wlan_ap_sta_deauth_t));
	os_memcpy(req->addr, mac, 6);
	req->reason = reason_code;

	return wpa_ctrl_request(WPA_CTRL_CMD_AP_STA_DEAUTH, req);
}

int wlan_ap_deauth_all_sta(void)
{
	return wpa_ctrl_request(WPA_CTRL_CMD_AP_DEAUTH_ALL_STA, NULL);
}
...
bk_err_t bk_wifi_sta_get_signal_info(wifi_signal_info_t *signal_info)
{
	...
	SIGNAL_INFO_PARAM_T *signal_info_param = (SIGNAL_INFO_PARAM_T *)os_malloc(sizeof(SIGNAL_INFO_PARAM_T));
	if (rw_msg_get_signal_info(vif_idx, signal_info_param) != 0) {
		os_free(signal_info_param);
		return BK_FAIL;
	} else {
		signal_info->frequency      = signal_info_param->frequency;
		signal_info->current_signal = signal_info_param->current_signal;
		signal_info->avg_signal     = signal_info_param->avg_signal;
		signal_info->current_noise  = signal_info_param->current_noise;
	}
	os_free(signal_info_param);
	return BK_OK;
}
...
#endif
#ifdef CONFIG_STA_MINMUM_SECURITY
	g_sta_param_ptr->sta_cipher_suite = config->security;
#endif
...
bk_err_t bk_wifi_sta_get_config(wifi_sta_config_t *config)
{
	...
	config->ssid[g_sta_param_ptr->ssid.length] = 0;
#if CONFIG_BSSID_CONNECT
	...
	// security may change if connected to AP
	config->security = g_sta_param_ptr->cipher_suite;
	config->channel = g_sta_param_ptr->fast_connect.chann;

	config->is_user_fast_connect = !g_is_get_fci_from_flash;
	config->is_not_support_auto_fci = !g_is_use_fci_from_flash;
...
#if CONFIG_STA_AUTO_RECONNECT
	config->auto_reconnect_count = g_sta_param_ptr->auto_reconnect_count;
	config->auto_reconnect_timeout = g_sta_param_ptr->auto_reconnect_timeout;
	config->disable_auto_reconnect_after_disconnect =
		g_sta_param_ptr->disable_auto_reconnect_after_disconnect;
#endif
...
#ifdef CONFIG_WIFI_WIRELESS_MODE_SUPPORT
void bk_wlan_sta_set_wireless_mode(uint8_t wireless_mode)
{
	if (wireless_mode == 0 || wireless_mode > 3) {
		WIFI_LOGE("invalid wireless_mode!\n");
		return;
	}
	g_sta_param_ptr->wireless_mode = wireless_mode;
	if (g_sta_param_ptr->wireless_mode != WIFI_WIRELESS_MODE_BGN) {
		rw_ieee80211_set_ht_cap(0);
	} else {
		rw_ieee80211_set_ht_cap(1);
	}
	rw_msg_send_wireless_mode_req(wireless_mode);
}
#endif

#ifdef CONFIG_WIFI_RETRY_CONFIG_SUPPORT
void bk_wlan_sta_set_retry_config(uint8_t auth_retry, uint16_t auth_retry_interval,
		uint8_t assoc_retry, uint16_t assoc_retry_interval, uint8_t eapol_2_retry) {
	g_sta_param_ptr->auth_retry = auth_retry;
	g_sta_param_ptr->assoc_retry = assoc_retry;
	g_sta_param_ptr->eapol_2_retry = eapol_2_retry;
	g_sta_param_ptr->auth_retry_interval = auth_retry_interval;
	g_sta_param_ptr->assoc_retry_interval = assoc_retry_interval;
	rw_msg_send_retry_config_req(auth_retry, auth_retry_interval, assoc_retry, assoc_retry_interval);
}
#endif

void bk_wlan_sta_get_tx_frame_count(uint32_t *tx_total_num, uint32_t *tx_dropped_num, uint32_t *tx_data_num) {
	struct mm_get_tx_frame_count_cfm *cfm = NULL;
	cfm = os_malloc(sizeof(struct mm_get_tx_frame_count_cfm));
	if (!cfm) {
		WIFI_LOGE("bk_wifi_get_tx_frame_count, malloc fail\r\n");
		goto get_exit;
	}

	if (BK_OK != rw_msg_get_tx_frame_count_req((void *)cfm)) {
		WIFI_LOGE("bk_wifi_get_tx_frame_count, send fail\r\n");
		goto get_exit;
	}

	*tx_total_num = cfm->tx_total_num;
	*tx_dropped_num = cfm->tx_dropped_num;
	*tx_data_num = cfm->tx_data_num;
...
void bk_wlan_sta_get_rx_frame_count(uint32_t *rx_total_num, uint32_t *rx_dropped_num, uint32_t *rx_data_num) {
	struct mm_get_rx_frame_count_cfm *cfm = NULL;
	cfm = os_malloc(sizeof(struct mm_get_rx_frame_count_cfm));
	if (!cfm) {
		WIFI_LOGE("bk_wifi_get_rx_frame_count, malloc fail\r\n");
		goto get_exit;
	}

	if (BK_OK != rw_msg_get_rx_frame_count_req((void *)cfm)) {
		WIFI_LOGE("bk_wifi_get_rx_frame_count, send fail\r\n");
		goto get_exit;
	}

	*rx_total_num = cfm->rx_total_num;
	*rx_dropped_num = cfm->rx_dropped_num;
	*rx_data_num = cfm->rx_data_num;
...
bk_err_t bk_wifi_sta_get_link_status(wifi_link_status_t *link_status)
{
	...
	os_memcpy(link_status->password, g_sta_param_ptr->key, g_sta_param_ptr->key_len);
...
#if CONFIG_BRIDGE
		ap->is_bridge = ap_list.ApList[i].is_bridge;
#endif
...
static bk_err_t wifi_ap_set_config(const wifi_ap_config_t *ap_config)
{
	...
	g_ap_param_ptr->bridge = ap_config->bridge;
	if (g_ap_param_ptr->key_len < 8) {
		g_ap_param_ptr->cipher_suite = WIFI_SECURITY_NONE;
	} else {
#if CONFIG_SOFTAP_WPA3
...
bk_err_t bk_wifi_monitor_set_channel(const wifi_channel_t *chan)
{
	...
	//always set to HT20
	rw_msg_set_channel(chan->primary, 0, PHY_CHNL_BW_20, NULL);
...
bk_err_t bk_wifi_monitor_set_channel_bonding(const wifi_channel_t *chan)
{
	...
	rw_msg_set_channel(chan->primary, chan->second, PHY_CHNL_BW_40, NULL);
...
__IRAM_SEC bk_err_t bk_wifi_get_wifi_media_mode_config(bool *flag)
{
	if (!flag)
		return BK_ERR_NULL_PARAM;
...
bk_err_t bk_wifi_send_sleep_mode_req(uint8_t sleep_mode) {
	if(sleep_mode > 2)
	{
		WIFI_LOGE("sleep mode can only be set 0, 1 or 2\n");
		return BK_FAIL;
	}
	...
	return rw_msg_send_sleep_mode_req(vif_idx, sleep_mode);
}

bk_err_t bk_wifi_get_sleep_mode(uint8_t *sleep_mode)
{
	bk_err_t get_result = BK_FAIL;
	struct mm_get_sleep_mode_cfm *cfm = NULL;

	WIFI_LOGD("bk_wifi_get_sleep_mode\n");

	cfm = os_malloc(sizeof(struct mm_get_sleep_mode_cfm));
	if (!cfm) {
		WIFI_LOGE("get_sleep_mode, malloc fail\r\n");
		goto get_exit;
	}

	if (BK_OK != rw_msg_get_sleep_mode_req((void *)cfm)) {
		WIFI_LOGE("get_sleep_mode, send fail\r\n");
		goto get_exit;
	}

	*sleep_mode = cfm->sleep_mode;
	get_result = BK_OK;
...
static wifi_bcnmiss_cb_t g_wifi_bcn_miss_cb = NULL;
bk_err_t bk_wifi_bcn_miss_register_cb(wifi_bcnmiss_cb_t bcnmiss_cb)
{
	g_wifi_bcn_miss_cb = bcnmiss_cb;
	return BK_OK;
}

bk_err_t bk_wifi_bcn_miss_cb(void)
{
	if (!g_wifi_bcn_miss_cb)
		return BK_OK;
	else
		return g_wifi_bcn_miss_cb();
}
...
void bk_wifi_set_microwave_policy_en(bool enable)
{
	bk_microwave_policy_en = enable;
}

bool bk_wifi_get_microwave_policy_en(void)
{
	return bk_microwave_policy_en;
}
...
#if CONFIG_ENCRYPT_WIFI_INFO
/* see decrypt_psk_README.md for more details */
#define BK_WIFI_ENCRYPT_KEY 	(0xAA)
static void bk_wifi_decrypt_array(const uint8_t *encrypted_array, int encrypted_len, uint8_t *decrypted_array)
{
	for (int i = 0; i < encrypted_len; i++) {
		decrypted_array[i] = encrypted_array[i] ^ BK_WIFI_ENCRYPT_KEY;
	}
}

uint8_t bk_wifi_get_encrypt_key(void)
{
	return BK_WIFI_ENCRYPT_KEY;
}

uint8_t* bk_wifi_encrypt_array(const uint8_t *data_array, int array_len)
{
	uint8_t *encrypted_array;

	if (data_array == NULL || array_len <= 0) {
		return NULL;
	}

	encrypted_array = (uint8_t *)os_malloc(array_len);
	if (encrypted_array == NULL) {
		return NULL;
	}

	// Encrypt each byte: simple XOR with the key
	bk_wifi_decrypt_array(data_array, array_len, encrypted_array);

	return encrypted_array;
}

uint8_t* bk_wifi_encrypt_string(const uint8_t *original_str, int original_len)
{
	int len = 0;
	int array_len = 0;
	int i;
	uint8_t *data_array = NULL;
	uint8_t *encrypted_array = NULL;
	uint8_t *encrypted_str = NULL;

	if (original_str == NULL) {
		return NULL;
	}

	len = os_strlen((char *)original_str);
	if(len > original_len) {
		len = original_len;
	}
	if (len == 0) {
	...
	array_len = len / 2;
	if (len % 2 != 0) {
		// Odd length, add one more byte for the last character
		array_len++;
	}

	data_array = (uint8_t *)os_malloc(array_len);
	if (data_array == NULL) {
		return NULL;
	}
	os_memset(data_array, 0, array_len);

	// Convert hex string to byte array
	for (i = 0; i < len; i++) {
		uint8_t hex_value = 0;
		uint8_t c = original_str[i];
		...
		if (i % 2 == 0) {
			// High nibble
			data_array[i / 2] = hex_value << 4;
		} else {
			// Low nibble
			data_array[i / 2] |= hex_value;
		}
	}

	// Encrypt the byte array

	encrypted_array = data_array;
	bk_wifi_decrypt_array(data_array, array_len, encrypted_array);

	// Convert encrypted byte array back to hex string
	encrypted_str = (uint8_t *)os_malloc(len + 1);
	if (encrypted_str == NULL) {
		os_free(data_array);
		return NULL;
	}
	os_memset(encrypted_str, 0, len + 1);

	for (i = 0; i < array_len; i++) {
		uint8_t byte_val = encrypted_array[i];
		...
		if (i * 2 < len) {
			if (high_nibble < 10) {
				encrypted_str[i * 2] = '0' + high_nibble;
			} else {
				encrypted_str[i * 2] = 'a' + (high_nibble - 10);
			}
		}
		...
		if (i * 2 + 1 < len) {
			if (low_nibble < 10) {
				encrypted_str[i * 2 + 1] = '0' + low_nibble;
			} else {
				encrypted_str[i * 2 + 1] = 'a' + (low_nibble - 10);
			}
		}
	}

	os_free(encrypted_array);
	encrypted_str[len] = '\0';

	return encrypted_str;
}
#endif //CONFIG_ENCRYPT_WIFI_INFO
