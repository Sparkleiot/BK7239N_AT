...
uint8_t vif_mgmt_get_sta_vif_index();
...
int rw_msg_send(const void *msg_params, int reqcfm, uint16_t reqid, void *cfm)
{
	...
	if (kNoErr != ret) {
		...
		// need to free the whole ke_msg not the msg_param part
		ke_msg_free(msg);
...
#undef MAC_BASIC_RATE
	...
	if (g_ap_param_ptr->cipher_suite > WIFI_SECURITY_WEP) {
...
int rw_msg_send_channel_update_req(uint8_t channel) {
	struct mm_chan_ctxt_update_req *req = KE_MSG_ALLOC(MM_CHAN_CTXT_UPDATE_REQ,
											TASK_MM, TASK_API, mm_chan_ctxt_update_req);
	req->chan_index = 0;
	req->chan.band = PHY_BAND_2G4;
	req->chan.flags = 0;
	req->chan.prim20_freq = req->chan.center1_freq = rw_ieee80211_get_centre_frequency(channel);
...
#if CONFIG_USER_SPECIFIC_SCAN
/**
 * sta_channel_mask is a 16-bit integer, it represents a WiFi channel from bit0 to bit 13
 * - Bit 0 corresponds to channel 1.
 * - Bit 1 corresponds to channel 2, and so on.
 * - A bit value of `1` indicates the channel is selected, while `0` means it is not.
 *
 * For example:
 * - A mask of `0b0000010000100001` indicates that channels 1, 6, and 11 are selected.
 * - The corresponding center frequencies are `2412 MHz`, `2437 MHz`, and `2462 MHz`.
 */
uint16_t sta_channel_mask = 0x3FFF;   // default 0x3FFF represents the full 2.4G channels

#define WIFI_CHANNEL_COUNT 14
int global_chan[WIFI_CHANNEL_COUNT] = {
	2412, 2417, 2422, 2427, 2432, 2437, 2442, 2447,
	2452, 2457, 2462, 2467, 2472, 2484
};

int select_center_freq_from_mask(unsigned int sta_channle_mask, int **freq) {
	int count = 0;
	for (int i = 0; i < WIFI_CHANNEL_COUNT; i++) {
		if (sta_channle_mask & (1 << i)) {
			count++;
		}
	}
	...
	*freq = (int *)os_malloc(count * sizeof(int));
	if (*freq == NULL) {
		return 0;
	}
...
#if CONFIG_USER_SPECIFIC_SCAN
	if (!freqs[0] && sta_channel_mask == 0X3FFF)
#else
	if (!freqs[0])
#endif
	{
...
#if CONFIG_USER_SPECIFIC_SCAN
	} else if (sta_channel_mask) {
		int *scan_freq = NULL;
		int freq_size = select_center_freq_from_mask(sta_channel_mask, &scan_freq);
		for (i = 0; i < freq_size; i++) {
			req->chan[i].band = IEEE80211_BAND_2GHZ;
			req->chan[i].flags = 0;
			req->chan[i].freq = scan_freq[i];
		}
		req->chan_cnt = i;
		os_free(scan_freq);
#endif
...
int rw_msg_get_signal_info(u8 vif_idx, SIGNAL_INFO_PARAM_T *signal_info_param)
{
	...
	signal_info_param->current_signal = (int)mac_vif_mgmt_get_u_sta_rssi(vif);
	signal_info_param->avg_signal = (int)mac_vif_mgmt_get_u_sta_smooth_rssi(vif);
...
int rw_msg_set_channel(uint32_t prim_channel, uint32_t second_channel, uint32_t band_width, void *cfm)
{
	...
	if (second_channel != 0) {
		//TODO: check if second_channel is valid for bonding
		set_chnl_par->chan.prim20_freq  = rw_ieee80211_get_centre_frequency(prim_channel);
		set_chnl_par->chan.center1_freq = rw_ieee80211_get_centre_frequency(second_channel);
	} else {
		set_chnl_par->chan.prim20_freq = set_chnl_par->chan.center1_freq =
											rw_ieee80211_get_centre_frequency(prim_channel);
	}
...
int rw_msg_send_wireless_mode_req(uint8_t wireless_mode)
{
	struct me_set_wireless_mode_req *req;
	req = ke_msg_alloc(ME_SET_WIRELESS_MODE_REQ, TASK_ME, TASK_API,
                          sizeof(struct me_set_wireless_mode_req));
	if (!req)
		return -ENOMEM;

	req->wireless_mode = wireless_mode;

	return rw_msg_send(req, 0, 0, NULL);
}

int rw_msg_send_retry_config_req(uint8_t auth_retry, uint16_t auth_retry_interval,
		uint8_t assoc_retry, uint16_t assoc_retry_interval)
{
	struct sm_set_retry_config_req *req;
	req = ke_msg_alloc(SM_SET_RETRY_CONFIG_REQ, TASK_SM, TASK_API,
                          sizeof(struct sm_set_retry_config_req));
	if (!req)
		return -ENOMEM;

	req->auth_retry = auth_retry;
	req->auth_retry_interval = auth_retry_interval;
	req->assoc_retry = assoc_retry;
	req->assoc_retry_interval = assoc_retry_interval;

	return rw_msg_send(req, 0, 0, NULL);
}

int rw_msg_get_tx_frame_count_req(void *cfm)
{
	struct mm_get_tx_frame_count_req *req = NULL;

	req = ke_msg_alloc(MM_GET_TX_FRAME_COUNT_REQ, TASK_MM, TASK_API,
					   sizeof(struct mm_get_tx_frame_count_req));

	if (!req)
		return BK_ERR_NO_MEM;

	return rw_msg_send(req, 1, MM_GET_TX_FRAME_COUNT_CFM, cfm);
}

int rw_msg_get_rx_frame_count_req(void *cfm)
{
	struct mm_get_rx_frame_count_req *req = NULL;

	req = ke_msg_alloc(MM_GET_RX_FRAME_COUNT_REQ, TASK_MM, TASK_API,
					   sizeof(struct mm_get_rx_frame_count_req));

	if (!req)
		return BK_ERR_NO_MEM;

	return rw_msg_send(req, 1, MM_GET_RX_FRAME_COUNT_CFM, cfm);
}
...
int rw_msg_send_sleep_mode_req(uint8_t vif_idx, uint8_t sleep_mode)
{
	struct mm_set_sleep_mode_req *req;
	/* Build the MM_SET_SLEEP_MODE_REQ message */
	req = ke_msg_alloc(MM_SET_SLEEP_MODE_REQ, TASK_MM, TASK_API,
                          sizeof(struct mm_set_sleep_mode_req));
	if (!req)
		return -ENOMEM;

	/* Set parameters */
	req->vif_index = vif_idx;
	req->sleep_mode = sleep_mode;

	/* Send the MM_SET_SLEEP_MODE_REQ message to LMAC FW */
	return rw_msg_send(req, 1, MM_SET_SLEEP_MODE_CFM, NULL);
}

int rw_msg_get_sleep_mode_req(void *cfm)
{
	struct mm_get_sleep_mode_req *req = NULL;

	req = ke_msg_alloc(MM_GET_SLEEP_MODE_REQ, TASK_MM, TASK_API,
					   sizeof(struct mm_get_sleep_mode_req));

	if (!req)
		return BK_ERR_NO_MEM;

	return rw_msg_send(req, 1, MM_GET_SLEEP_MODE_CFM, cfm);
}
