...
void rwnx_upload_amsdu(struct fhost_rx_header *rxhdr)
{
	...
	while (q) {
		...
		if (q->len > sizeof(struct ethhdr)) {
			struct ethhdr *ethhdr = (struct ethhdr *)q->payload;
			// for 802.3 frame, pass it to lwip, if the frame's src addr not equal to us.
			if (vif_entry && memcmp(&ethhdr->src, mac_vif_mgmt_get_mac_addr(vif_entry), ETH_ALEN)) {
...
//#include "mac.h"
bool compare_mac_addresses(const char *mac1, uint8_t *mac2_input) {
    unsigned char mac1_bytes[6];
    struct mac_addr *mac2 = (struct mac_addr *)mac2_input;
    if (sscanf(mac1, "%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx",
               &mac1_bytes[0], &mac1_bytes[1], &mac1_bytes[2],
               &mac1_bytes[3], &mac1_bytes[4], &mac1_bytes[5]) != 6) {
        return false;
    }

    uint16_t mac1_uint16[3];
    mac1_uint16[0] = (mac1_bytes[1] << 8) | mac1_bytes[0];
    mac1_uint16[1] = (mac1_bytes[3] << 8) | mac1_bytes[2];
    mac1_uint16[2] = (mac1_bytes[5] << 8) | mac1_bytes[4];

    for (int i = 0; i < 3; i++) {
        if (mac1_uint16[i] != mac2->array[i]) {
            return false;
        }
    }
...
int8_t get_rssi_by_mac(char *mac)
{
    int8_t rssi = 0XFF;
    void  *sta_entry = NULL;
    uint8 *sta_mac_addr = NULL;

    for (int i = 0; i < STA_MAX; i++) {
        sta_entry = sta_mgmt_get_entry(i);
        sta_mac_addr = sta_mgmt_get_mac_addr(sta_entry);
        if (compare_mac_addresses(mac, sta_mac_addr))
            rssi = sta_mgmt_get_rssi(sta_entry);
    }

    if (rssi == 0XFF)
        os_printf("input mac address is invalid\r\n");
    else
        os_printf("mac:%pM rssi=%d\r\n", mac, rssi);

    return rssi;
}

#ifdef CONFIG_WIFI6
// Return 0 let icmp module choose response len.
int icmp_resp_len(struct pbuf *p)
{
	int8_t modf, mcs, bw;
	struct fhost_rx_header *rxhdr;
	const int ac_20M_byte_per_16symbol[10]  = { 256, 256, 256, 256, 512, 512, 512, 512, 768, 1024};
	const int ac_40M_byte_per_16symbol[10]  = { 256, 256, 512, 512, 768, 1024, 1024, 1024, 1280, 1536};
	const int ax_20M_n_byte_per_16symbol[10] = { 231, 465, 699, 933, 1401, 1869, 2103, 2337, 2805, 3117};
	...
	bw = (rxhdr->hwvect.recvec1a & 0b110000) >> 4; // hal_desc_get_bandwdith
	if (modf < 2) {
	...
	if (modf == 4 && mcs >= 0 && mcs < ARRAY_SIZE(ac_40M_byte_per_16symbol)) {
		// VHT 20M/40M
		if (bw == 0)
			return ac_20M_byte_per_16symbol[mcs];
		else if (bw == 1)
			return ac_40M_byte_per_16symbol[mcs];
	} else if (modf >= 5 && mcs >= 0 && mcs < ARRAY_SIZE(ax_20M_n_byte_per_16symbol)) {
		// HE 20M/40M
		if (bw == 0)
			return ax_20M_n_byte_per_16symbol[mcs];
		else if (bw == 1)
			return ax_20M_n_byte_per_16symbol[mcs] << 1;
	}
...
UINT32 rwm_upload_data(void *host_id, uint32_t frame_len)
{
	...
	// Don't update pbuf len/tot_len here, because frame len is
	// the whole length of AMSDU packet instead of first MSDU of AMSDU.
	// Else pbuf_clone will assert because pbuf->len is larger than
	// pbuf->tot_len. (AMY-697).
	rxhdr = (struct fhost_rx_header *)q->payload;
	q->extra = rxhdr = (struct fhost_rx_header *)q->payload;
	...
	if (rxhdr->flags_is_80211_mpdu) {
		...
		if (q->len > sizeof(struct ethhdr))
		{
			...
			// for 802.3 frame, pass it to lwip, if the frame's src addr not equal to us.
			if (vif_entry && os_memcmp(&ethhdr->src, mac_vif_mgmt_get_mac_addr(vif_entry), ETH_ALEN)) {
