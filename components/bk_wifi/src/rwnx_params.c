...
struct rwnx_mod_params rwnx_mod_params = {
	...
	PARAM(tx_agg_num, 0),
	PARAM(tx_use_max_bw, false),
	PARAM(amsdu_require_spp, false)
};

static const int mcs_map_to_rate[4][3] __maybe_unused = {
...
static void rwnx_set_ht_capa(struct rwnx_hw *rwnx_hw, struct wiphy *wiphy)
{
	...
	for (i = 0; i < nss; i++) {
		...
		if(bk_wifi_get_microwave_policy_en())
			rwnx_hw->mod_params->ht_mcs_map = 0xF;
		#endif
		band_2GHz->ht_cap.mcs.rx_mask[i] = rwnx_hw->mod_params->ht_mcs_map;
	}
...
void rwnx_udpate_capability(uint32_t capa_id, uint32_t capa_val)
{
	switch (capa_id)
	{
		...
		case WIFI_CAPA_ID_RX_AMPDU_NUM:
			...
			break;
		case WIFI_CAPA_ID_TX_USE_MAX_BW:
			rwnx_update_tx_use_max_bw(capa_val);
			break;
...
bool rwnx_mod_param_get_use_2040()
{
	return rwnx_mod_params.use_2040;
}

bool rwnx_get_tx_use_max_bw(void)
{
    return rwnx_mod_params.tx_use_max_bw;
}

/**
 ****************************************************************************************
 * @brief Get beacon loss count
 *
 * @return Beacon loss count
 ****************************************************************************************
 */
 #if CONFIG_MICROWAVE_POLICY
 UINT32 rwnx_get_beacon_loss_count(void)
 {
	 if (bk_wifi_get_microwave_policy_en())
		 return 600;
	 else
