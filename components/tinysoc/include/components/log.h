...
#define BK_LOGE(tag, format, ...) if (is_err_log_enabled()) _OS_PRINTF(BK_LOG_FORMAT(E, format), rtos_get_time(), tag, ## __VA_ARGS__)
#else
#define BK_LOGE(tag, format, ...) do { (void)sizeof(tag, format, ## __VA_ARGS__); } while (0)
#endif

#if (LOG_LEVEL >= BK_LOG_WARN)
#define BK_LOGW(tag, format, ...) if (is_err_log_enabled()) _OS_PRINTF(BK_LOG_FORMAT(W, format), rtos_get_time(), tag, ## __VA_ARGS__)
#else
#define BK_LOGW(tag, format, ...) do { (void)sizeof(tag, format, ## __VA_ARGS__); } while (0)
#endif

#if (LOG_LEVEL >= BK_LOG_INFO)
#define BK_LOGI(tag, format, ...) if (is_info_log_enabled()) _OS_PRINTF(BK_LOG_FORMAT(I, format), rtos_get_time(), tag, ## __VA_ARGS__)
#else
...
#define BK_LOG_RAW(format, ...) if (is_info_log_enabled()) _OS_PRINTF(format, ##__VA_ARGS__)
