...
static inline void sys_hal_set_low_voltage(volatile uint32_t *ana_r0, volatile uint32_t *ana_r3, volatile uint32_t *ana_r7, volatile uint32_t *ana_r8, volatile uint32_t *ana_r9, volatile uint32_t *ana_r10)
{
	...
	*ana_r10 = sys_ll_get_ana_reg10_value();
	...
	sys_ll_set_ana_reg7_vanaldosel(0);//bit[9:6]
#if CONFIG_LDO_SELF_LOW_POWER_MODE_ENA
	sys_ll_set_ana_reg7_dldohp(0);//bit2
	sys_ll_set_ana_reg7_aldohp(0);//bit10
#endif
	sys_ll_set_ana_reg7_bypassen(1);//bit20
	sys_ll_set_ana_reg7_ioldolp(1);//bit21

	sys_ll_set_ana_reg9_lburstsel(3);//bit[19:18]
	sys_ll_set_ana_reg9_mrosci_cal(0);//bit[27:25]
	sys_ll_set_ana_reg10_avea_sel(3);
	sys_ll_set_ana_reg10_arampc(0);
...
/*-----------config wakeup source  end --------------*/
	...
	sys_ll_set_ana_reg8_vcorehsel(PM_LOW_VOL_VCOREHSEL_LDO_SEL); //0x0:0.7V vcore
	sys_hal_disable_spi_latch();
#endif
...
/*----enter low voltage sleep-------*/
	...
	//sys_hal_set_low_voltage_sleep_duration_us(bk_aon_rtc_get_us() - before);
	sys_hal_set_low_voltage_sleep_duration_us(1);
...
{
	sys_ll_set_ana_reg8_spi_latch1v(1);
	sys_ll_set_ana_reg7_vanaldosel(4);
	sys_ll_set_ana_reg8_spi_latch1v(0);
...
{
	sys_ll_set_ana_reg8_spi_latch1v(1);
	sys_ll_set_ana_reg7_vanaldosel(0xF);
	sys_ll_set_ana_reg8_spi_latch1v(0);
...
void sys_hal_low_power_hardware_init()
{
	...
	/*set the lp voltage*/
	sys_hal_lp_vol_set(0x7); // core 0.625V
