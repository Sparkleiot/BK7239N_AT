...
static void flash_mpc_cfg(void)
{
	...
	uint32_t ota_phy_offset = partition_get_phy_offset(PARTITION_XIP_A);
	uint32_t ota_phy_size = partition_get_phy_size(PARTITION_XIP_B);
#endif
	uint32_t ota_vir_offset = FLASH_PHY2VIRTUAL(ota_phy_offset);
	uint32_t ota_vir_size = FLASH_PHY2VIRTUAL(ota_phy_size);
	uint32_t ns_block_num = (ota_vir_offset - nspe_vir_offset) / block_sz;
	...
	BK_LOG_ON_ERR(bk_mpc_set_secure_attribute(MPC_DEV_FLASH, nspe_vir_offset, ns_block_num, MPC_BLOCK_NON_SECURE));

	ns_block_num = max_block_num - ((ota_vir_offset + ota_vir_size) / block_sz);
	BK_LOGI(TAG, "max_block_num=%x, offset=%x, ns_block_num=%x\r\n", max_block_num, ota_vir_offset + ota_vir_size, ns_block_num);
	BK_LOG_ON_ERR(bk_mpc_set_secure_attribute(MPC_DEV_FLASH, ota_vir_offset + ota_vir_size, ns_block_num, MPC_BLOCK_NON_SECURE));
...
static void ram_mpc_cfg(void)
{
...
#endif
	// uint32_t total_remain_s_size = CONFIG_TFM_RAM_SIZE + 0x20000;
	...
	for (uint32_t dev = MPC_DEV_SMEM0; dev < MPC_DEV_MAX; dev ++) {
