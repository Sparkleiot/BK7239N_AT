...
static int http_on_header_value(http_parser *parser, const char *at, size_t length)
{
	...
	} else if (strcasecmp(client->current_header_key, "ETag") == 0) {
		http_utils_append_string(&client->etag, at, length);
	}
...
bk_err_t bk_http_check_response(bk_http_client_handle_t client)
{
	bk_err_t err = BK_FAIL;
	
	if (client->response->status_code < HttpStatus_Ok) {
		BK_LOGE(TAG, "Invalid HTTP status: %d\r\n", client->response->status_code);
		return BK_FAIL;
	}

	if (redirection_required(client->response->status_code)) {
		err = bk_http_client_set_redirection(client);
		if (err != BK_OK) {
			BK_LOGE(TAG, "URL redirection Failed");
			return err;
		}
	} else if (client->response->status_code == HttpStatus_NotModified) {
		return BK_FAIL;
	} else if (client->response->status_code == HttpStatus_Unauthorized) {
		BK_LOGE(TAG, "HttpStatus_Unauthorized\r\n");
		bk_http_client_add_auth(client);
	} else if(client->response->status_code == HttpStatus_NotFound || client->response->status_code == HttpStatus_Forbidden) {
		BK_LOGE(TAG, "File not found(%d)", client->response->status_code );
		return BK_FAIL;
	} else if (client->response->status_code >= HttpStatus_BadRequest && client->response->status_code < HttpStatus_InternalError) {
		BK_LOGE(TAG, "Client error (%d)", client->response->status_code );
		return BK_FAIL;
	} else if (client->response->status_code >= HttpStatus_InternalError) {
		BK_LOGE(TAG, "Server error (%d)", client->response->status_code );
		return BK_FAIL;
	}
	else {
		BK_LOGE(TAG, "status_code: (%d)", client->response->status_code );
	}
	return BK_OK;
}
