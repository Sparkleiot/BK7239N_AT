...
void sys_hal_rf_ctrl(uint8_t type)
{
    if (type == RF_CTRL_PTA) {
        // rf ctrl by PTA
        sys_ll_set_cpu_storage_connect_op_select_rf_switch_en(0);
        sys_ll_set_cpu_storage_connect_op_select_rf_for_wifiorbt(0);
    } else if (type == RF_CTRL_WIFI) {
        // rf ctrl by Wi-Fi
        sys_ll_set_cpu_storage_connect_op_select_rf_for_wifiorbt(0);
        sys_ll_set_cpu_storage_connect_op_select_rf_switch_en(1);
    } else {
        // rf ctrl by BT
        sys_ll_set_cpu_storage_connect_op_select_rf_for_wifiorbt(1);
        sys_ll_set_cpu_storage_connect_op_select_rf_switch_en(1);
    }
}
...
{
    uint8_t type = 0;
    if (sys_ll_get_cpu_storage_connect_op_select_rf_switch_en()) {
        if (sys_ll_get_cpu_storage_connect_op_select_rf_for_wifiorbt()) {
            type = RF_CTRL_BT;
        } else {
            type = RF_CTRL_WIFI;
        }
...
{
	sys_ll_set_sys_debug_config1_value(type);
}

void sys_hal_diag_debug_mac()
{
	sys_ll_set_sys_debug_config1_value(DIAG_DEBUG_MAC);
}

void sys_hal_diag_debug_phy()
{
	sys_ll_set_sys_debug_config1_value(DIAG_DEBUG_PHY);
}
...
{
	sys_ll_set_ana_reg10_vddgpio_sel(value); //GPIO vdd select; �� default��0��0:vddio;1:vddpsram
}
...
#define FLASH_DPLL_CLK    120    // 120MHz
#define CLK_XTAL    26    // 26MHz


void sys_hal_set_flash_clk(uint32_t flash_clk)
{
	uint32_t flash_max_clk = FLASH_DPLL_CLK;
	uint32_t calculated_div = flash_max_clk / flash_clk;
	...
	if (flash_clk <= CLK_XTAL) {
		...
		if (calculated_div <= 4) {          //0:/4  1:/6  2:/8  3:/10，it is invalid when cksel_flash=0
			flash_div = 0;
		} else if (calculated_div <= 6) {
			flash_div = 1;
		} else if (calculated_div <= 8) {
			flash_div = 2;
		} else {
			flash_div = 3;
		}
