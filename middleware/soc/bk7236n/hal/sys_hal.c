...
__attribute__((section(".uninitialized"))) static uint32_t boot_ana_rtc_int_status = 0;
__attribute__((section(".uninitialized"))) static uint32_t boot_ana_gpio_int_status = 0;
...
bk_err_t sys_hal_init()
{
	...
	sys_ana_ll_set_ana_reg10_vbg_rstrtc_en(0);
#else
...
__IRAM_SEC void sys_hal_module_power_ctrl(power_module_name_t module,power_module_state_t power_state)
{
	...
	if((module >= POWER_MODULE_NAME_MEM1) && (module <= POWER_MODULE_NAME_ROM_PGEN)) {
		...
		if((chip_id & PM_CHIP_ID_MASK) == (PM_CHIP_ID_MPW_V3 & PM_CHIP_ID_MASK))
		{
			bk_delay_us(40);
		}
		else
...
bk_err_t sys_hal_switch_cpu_bus_freq_high_to_low(pm_cpu_freq_e cpu_bus_freq)
{
	...
	switch(cpu_bus_freq)
	{
		...
		case PM_CPU_FRQ_160M://cpu0:160m;cpu1:xm;;cpu2:xm;bus:240m
		...
		case PM_CPU_FRQ_XTAL://cpu0:26m;cpu1:26m;cpu2:26m;bus:26m
...
__IRAM_SEC void sys_hal_sadc_pwr_up(void)
{
...
__IRAM_SEC void sys_hal_en_tempdet(uint32_t value)
{
...
{
    uint32_t reg_val = sys_ana_ll_get_reg5_value();
    if (high_volt_mode) {
        reg_val &= ~(0x01 << 3);
    } else {
        reg_val |= (0x01 << 3);
...
{
	sys_ll_set_dbug_config0_dbug_mux(type);
}

void sys_hal_diag_debug_mac()
{
	sys_ll_set_dbug_config0_dbug_mux(DIAG_DEBUG_MAC);
}

void sys_hal_diag_debug_phy()
{
	sys_ll_set_dbug_config0_dbug_mux(DIAG_DEBUG_PHY);
}

// Set GPIO Mux for Debug Purpose.
...
__IRAM_SEC uint32_t sys_hal_analog_get_iram(analog_reg_t reg)
{
    uint32_t analog_reg_address;
	...
	return sys_ll_get_analog_reg_value_iram(analog_reg_address);
}
...
{
	sys_ll_set_ana_reg8_spi_latch1v_iram(value);
}
...
__IRAM_SEC void sys_hal_set_ana_reg7_timer_wkrstn(uint32_t value)
{
	sys_ll_set_ana_reg7_timer_wkrstn(value);
}

__IRAM_SEC void sys_hal_set_ana_reg8_rst_timerwks1v(uint32_t value)
{
	sys_ll_set_ana_reg8_rst_timerwks1v(value);
}

__IRAM_SEC void sys_hal_set_ana_reg8_rst_gpiowks(uint32_t value)
{
	sys_ll_set_ana_reg8_rst_gpiowks(value);
}
...
__IRAM_SEC uint32_t sys_hal_get_ana_reg11_rtcsel(void)
{
	return sys_ll_get_ana_reg11_rtcsel();
}

__IRAM_SEC void sys_hal_set_ana_reg11_rtcsel(uint32_t value)
{
	sys_ll_set_ana_reg11_rtcsel(value);
}

__IRAM_SEC uint32_t sys_hal_get_ana_reg11_timersel(void)
{
	return sys_ll_get_ana_reg11_timersel();
}

__IRAM_SEC void sys_hal_set_ana_reg11_timersel(uint32_t value)
{
	sys_ll_set_ana_reg11_timersel(value);
}

__IRAM_SEC uint32_t sys_hal_get_ana_reg12_timersel(void)
{
	return sys_ll_get_ana_reg12_timersel();
}

__IRAM_SEC void sys_hal_set_ana_reg12_timersel(uint32_t value)
{
	sys_ll_set_ana_reg12_timersel(value);
}

__IRAM_SEC uint32_t sys_hal_get_ana_reg13_rtcsel(void)
{
	return sys_ll_get_ana_reg13_rtcsel();
}

__IRAM_SEC void sys_hal_set_ana_reg13_rtcsel(uint32_t value)
{
...
static void sys_hal_dpll_cpu_flash_time_early_init(uint32_t chip_id)
{
	...
	if((coresel != PM_CLKSEL_CORE_480M) || !sys_ll_get_ana_reg1_manual())
	{
		sys_hal_cali_dpll(1);
	}
...
static void sys_hal_get_boot_wakeup_int_status(void)
{
    uint32_t check_rtc_int_en = sys_ll_get_cpu0_int_32_63_en_cpu0_ana_rtc_int_en();
    uint32_t check_gpio_int_en = sys_ll_get_cpu0_int_32_63_en_cpu0_ana_gpio_int_en();

    if (check_rtc_int_en == 0) {
        sys_ll_set_cpu0_int_32_63_en_cpu0_ana_rtc_int_en(1);
    }
    if (check_gpio_int_en == 0) {
        sys_ll_set_cpu0_int_32_63_en_cpu0_ana_gpio_int_en(1);
    }

    boot_ana_rtc_int_status = sys_ll_get_cpu0_int_32_63_status_cpu0_ana_rtc_int_st();
    boot_ana_gpio_int_status = sys_ll_get_cpu0_int_32_63_status_cpu0_ana_gpio_int_st();

    if (check_rtc_int_en == 0) {
        sys_ll_set_cpu0_int_32_63_en_cpu0_ana_rtc_int_en(0);
    }
    if (check_gpio_int_en == 0) {
        sys_ll_set_cpu0_int_32_63_en_cpu0_ana_gpio_int_en(0);
    }
}

uint32_t sys_hal_get_boot_ana_rtc_int_status(void)
{
	return boot_ana_rtc_int_status;
}

uint32_t sys_hal_get_boot_ana_gpio_int_status(void)
{
	return boot_ana_gpio_int_status;
}
...
void sys_hal_early_init(void)
{
	...
	/* sys_persist_check_crc costs about 3 ms */
	if (sys_persist_check_crc()) {
		bk_misc_set_reset_reason(RESET_SOURCE_ILLEGAL_PERSIST_DATA);
		bk_reboot_ex(RESET_SOURCE_ILLEGAL_PERSIST_DATA);
		return;
	}

	sys_persist_sys_t *sys_ana = get_sys_persist_sys();
	sys_hal_analog_set(ANALOG_REG0, sys_ana->r0);
	sys_hal_analog_set(ANALOG_REG1, sys_ana->r1);
	sys_hal_analog_set(ANALOG_REG2, sys_ana->r2);
	sys_hal_analog_set(ANALOG_REG3, sys_ana->r3);
	sys_hal_analog_set(ANALOG_REG4, sys_ana->r4);
	sys_hal_analog_set(ANALOG_REG5, sys_ana->r5);
	sys_hal_analog_set(ANALOG_REG6, sys_ana->r6);
	sys_hal_analog_set(ANALOG_REG7, sys_ana->r7);
	sys_hal_analog_set(ANALOG_REG8, sys_ana->r8);

	// keep the rtc int status before RTC_KEEP_TIME_SUPPORT restore the time
	sys_hal_get_boot_wakeup_int_status();
	...
	sys_hal_analog_set(ANALOG_REG9, 0xB55D9327);//tenglong20240805:2M buck for EVM;tenglong20251011<13>=1
#else
	sys_hal_analog_set(ANALOG_REG9, sys_ana->r9);//tenglong20240805:2M buck for EVM
#endif
	sys_hal_analog_set(ANALOG_REG10, sys_ana->r10);
	sys_hal_analog_set(ANALOG_REG11, sys_ana->r11);
	sys_hal_analog_set(ANALOG_REG12, sys_ana->r12);
	sys_hal_analog_set(ANALOG_REG13, sys_ana->r13);
	sys_hal_analog_set(ANALOG_REG14, sys_ana->r14);
	sys_hal_analog_set(ANALOG_REG15, sys_ana->r15);
	sys_ll_set_ana_reg8_spi_latch1v(0);
...
#define FLASH_MAX_CLK    120    // 120MHz
#define FLASH_80M_CLK    80    // 80MHz
#define CPU_CLK_XTAL    40    // 40MHz

void sys_hal_set_flash_clk(uint32_t flash_clk)
{
	uint32_t flash_max_clk = FLASH_MAX_CLK;
	uint32_t flash_80m_clk = FLASH_80M_CLK;
	uint32_t calculated_div = flash_80m_clk / flash_clk;
	uint32_t flash_div = 0;

	if (flash_clk == CPU_CLK_XTAL) {
		sys_ll_set_cpu_clk_div_mode2_cksel_flash(0);
	} else if (flash_clk == 80) {
		sys_ll_set_cpu_clk_div_mode2_cksel_flash(2);
		if (calculated_div == 1) {          //0:/1  1:/2  2:/4  3:8，it is invalid when cksel_flash=0 and 1
			flash_div = 0;
		} else if (calculated_div == 2) {
			flash_div = 1;
		} else if (calculated_div == 4) {
			flash_div = 2;
		} else {
			flash_div = 3;
		}
		sys_ll_set_cpu_clk_div_mode2_ckdiv_flash(flash_div);
	} else {
		calculated_div = flash_max_clk / flash_clk;
...
void sys_hal_set_cpu_clk(uint32_t cpu_clk)
{
	if (cpu_clk <= CPU_CLK_XTAL) {
		sys_hal_switch_cpu_bus_freq(PM_CPU_FRQ_XTAL);
	} else if (cpu_clk <= 60) {
		sys_hal_switch_cpu_bus_freq(PM_CPU_FRQ_60M);
	} else if (cpu_clk <= 80) {
		sys_hal_switch_cpu_bus_freq(PM_CPU_FRQ_80M);
	} else if (cpu_clk <= 120) {
		sys_hal_switch_cpu_bus_freq(PM_CPU_FRQ_120M);
	} else if (cpu_clk <= 160) {
		sys_hal_switch_cpu_bus_freq(PM_CPU_FRQ_160M);
	} else {
		sys_hal_switch_cpu_bus_freq(PM_CPU_FRQ_240M);
	}
}

__IRAM_SEC void sys_hal_enable_pll_iram(void)
{
	uint32_t ana_reg1_val = REG_READ(0x2807e400 + (1 << 2));  // restore sys0x41 from ram
	uint32_t ana_reg5_val = sys_hal_analog_get(5);  // sys0x45 corresponds to ana_reg_5
	sys_hal_analog_set(1, ana_reg1_val);

	// 1. Enable PLL (set bit 5 of ana_reg_5)
	ana_reg5_val |= (1 << 5);  // Set bit 5
	sys_hal_analog_set(5, ana_reg5_val);

	// 2. Synchronize ASIC
	sys_ll_set_cpu_device_clk_enable_dplldiv_cken(1);
}

__IRAM_SEC void sys_hal_enable_high_freq_iram(void)
{
	sys_ll_set_cpu_clk_div_mode2_ckdiv_flash(0x0);
	sys_ll_set_cpu_clk_div_mode2_cksel_flash(0x2);//some flash ,such as : flash mid = 0xC86516 ,max freq is 104M
