...
#if CONFIG_SOC_BK7239N_SMP_TEMP
#include "cpu_id.h"
#endif
#define TAG "pm"
...
__IRAM_SEC void sys_hal_module_power_ctrl(power_module_name_t module,power_module_state_t power_state)
{
	...
	if((module >= POWER_MODULE_NAME_MEM1) && (module <= POWER_MODULE_NAME_ROM_PGEN)) {
		...
		bk_delay_us(40);// V3 delay 200us, V4 delay 0us, V5 delay 40us
...
/**  SCR End  */
void  sys_hal_set_cpu_cp_test_config(uint32_t cfg_id)
{
	uint32_t spi_latchlb;
	uint32_t vorehsel;
	switch (cfg_id) {
	case 0:
		//cpu clk 160MHz
		sys_ll_set_cpu_clk_div_mode1_cksel_core(0x1);  // select DPLL480MHz
		sys_ll_set_cpu_clk_div_mode1_clkdiv_core(0x2);  // DPLL480M/(1 + 2) = 160Mhz

		// volt 0.755v
		spi_latchlb = sys_ana_ll_get_reg8_value();
		spi_latchlb |= (0x1 << 9);
		sys_ana_ll_set_reg8_value(spi_latchlb);
		vorehsel = sys_ana_ll_get_reg8_value();
		vorehsel = (vorehsel & ~(0xF << 16)) | (0x7 << 16);
		sys_ana_ll_set_reg8_value(vorehsel);
		spi_latchlb = sys_ana_ll_get_reg8_value();
		spi_latchlb &= ~(0x1 << 9);
		sys_ana_ll_set_reg8_value(spi_latchlb);

		// sram rate 3
		sys_ll_set_cpu_power_sleep_wakeup_share_mem_clkgating_disable(0x1);
		sys_ll_set_cpu_device_mem_ctrl3_ram_config(0x233);
		sys_ll_set_cpu_power_sleep_wakeup_share_mem_clkgating_disable(0x0);

		break;
	case 1:
		//cpu clk 240MHz
		sys_ll_set_cpu_clk_div_mode1_cksel_core(0x1);  // select DPLL480MHz
		sys_ll_set_cpu_clk_div_mode1_clkdiv_core(0x1);  // DPLL480M/(1 + 1) = 240Mhz
		...
		sys_ana_ll_set_reg8_value(spi_latchlb);
		vorehsel = sys_ana_ll_get_reg8_value();
		vorehsel = (vorehsel & ~(0xF << 16)) | (0xB << 16);
		sys_ana_ll_set_reg8_value(vorehsel);
		spi_latchlb = sys_ana_ll_get_reg8_value();
		spi_latchlb &= ~(0x1 << 9);
		sys_ana_ll_set_reg8_value(spi_latchlb);
		...
		// sram rate 1
		sys_ll_set_cpu_power_sleep_wakeup_share_mem_clkgating_disable(0x1);
		sys_ll_set_cpu_device_mem_ctrl3_ram_config(0x221);
		sys_ll_set_cpu_power_sleep_wakeup_share_mem_clkgating_disable(0x0);
