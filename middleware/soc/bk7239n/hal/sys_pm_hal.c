...
static inline void sys_hal_set_low_voltage(volatile uint32_t *ana_r0, volatile uint32_t *ana_r3, volatile uint32_t *ana_r7, volatile uint32_t *ana_r8, volatile uint32_t *ana_r9, volatile uint32_t *ana_r10)
{
	...
	*ana_r10 = sys_ana_ll_get_ana_reg10_value();
	...
	sys_ana_ll_set_ana_reg7_vanaldosel((s_pm_config.lvsleep_vanaldosel == 0xff) ? 2 : s_pm_config.lvsleep_vanaldosel); // 0: 0.9V, 2: 1V
#if CONFIG_TEMP_DETECT
	float temp;
	...
	if((err != BK_OK) || (temp < 0)) // get temperature failed or temperature is less than 0
	{
		...
		sys_ana_ll_set_ana_reg7_dldohp((s_pm_config.lvsleep_dldohp_low_temp == 0xff) ? 1 : s_pm_config.lvsleep_dldohp_low_temp); //will increase current 15uA
		sys_ana_ll_set_ana_reg7_aldohp((s_pm_config.lvsleep_aldohp_low_temp == 0xff) ? 0 : s_pm_config.lvsleep_aldohp_low_temp);
		#endif
	}
	else
	{
		#if CONFIG_LDO_SELF_LOW_POWER_MODE_ENA
		sys_ana_ll_set_ana_reg7_dldohp((s_pm_config.lvsleep_dldohp_high_temp == 0xff) ? 0 : s_pm_config.lvsleep_dldohp_high_temp);
		sys_ana_ll_set_ana_reg7_aldohp((s_pm_config.lvsleep_aldohp_high_temp == 0xff) ? 0 : s_pm_config.lvsleep_aldohp_high_temp);
		#endif
	}
#else
	#if CONFIG_LDO_SELF_LOW_POWER_MODE_ENA
	...
	#endif
#endif
	sys_ana_ll_set_ana_reg7_bypassen((s_pm_config.lvsleep_bypassen == 0xff) ? 1 : s_pm_config.lvsleep_bypassen);//bit20
	sys_ana_ll_set_ana_reg7_ioldolp((s_pm_config.lvsleep_ioldolp == 0xff) ? 1 : s_pm_config.lvsleep_ioldolp);//bit21

	// sys_ana_ll_set_ana_reg9_azcd_manu(0x20);
	// sys_ana_ll_set_ana_reg9_enzcdcalib(0);
	// sys_ana_ll_set_ana_reg9_zcdmsel(1);

	sys_ana_ll_set_ana_reg9_iburstsel((s_pm_config.lvsleep_iburstsel == 0xff) ? 3 : s_pm_config.lvsleep_iburstsel);//bit[19:18]
	sys_ana_ll_set_ana_reg9_mrosci_cal((s_pm_config.lvsleep_mrosci_cal == 0xff) ? 0 : s_pm_config.lvsleep_mrosci_cal);//bit[27:25]
	sys_ana_ll_set_ana_reg10_avea_sel((s_pm_config.lvsleep_avea_sel == 0xff) ? 3 : s_pm_config.lvsleep_avea_sel);
...
/*-----------config wakeup source  end --------------*/
	...
	sys_ana_ll_set_ana_reg8_vcorehsel((s_pm_config.lvsleep_vcorehsel == 0xff) ? PM_LOW_VOL_VCOREHSEL_LDO_SEL : s_pm_config.lvsleep_vcorehsel); //0x0:0.7V vcore
	sys_hal_disable_spi_latch();
#endif
...
static bk_err_t sys_hal_normal_sleep_ultra_low_power_exit(pm_normal_sleep_mode_e normal_sleep)
{
	uint32_t value = 0;
	/*latch*/
...
static bk_err_t sys_hal_normal_sleep_mode_1_enter(pm_normal_sleep_mode_e normal_sleep)
{
	uint32_t value = 0;
	/*latch*/
	value = sys_ana_ll_get_reg8_value();
	...
	/* cpu freq use 160M*/
	sys_ll_set_cpu_clk_div_mode1_clkdiv_core(2);
	sys_ll_set_cpu_clk_div_mode1_cksel_core(1);
...
bk_err_t sys_hal_normal_sleep_mode_ctrl(pm_normal_sleep_mode_e normal_sleep)
{
	switch(normal_sleep)
	{
		case PM_NORMAL_SLEEP_ULTRA_EXIT:
		{
			sys_hal_normal_sleep_ultra_low_power_exit(normal_sleep);
		}
		break;
		...
		{
			sys_hal_normal_sleep_mode_1_enter(normal_sleep);
		}
		break;
		default:
		break;
...
void sys_hal_enter_low_analog(void)
{
	...
	{
		sys_hal_set_vanaldosel((s_pm_config.btpll_vanaldosel_low == 0xff) ? 0x7 : s_pm_config.btpll_vanaldosel_low);
	}
	else
	{
		sys_hal_set_vanaldosel((s_pm_config.btpll_vanaldosel_high == 0xff) ? 0xa : s_pm_config.btpll_vanaldosel_high);
	...
	sys_ana_ll_set_ana_reg3_hpssren((s_pm_config.btpll_hpssren == 0xff) ? 0 : s_pm_config.btpll_hpssren);
	/* tx low performence 0x45[11][9] = 1*/
	sys_ana_ll_set_ana_reg5_anabufsel_tx1((s_pm_config.btpll_anabufsel_tx1 == 0xff) ? 0 : s_pm_config.btpll_anabufsel_tx1);
	sys_ana_ll_set_ana_reg5_anabufsel_tx0((s_pm_config.btpll_anabufsel_tx0 == 0xff) ? 1 : s_pm_config.btpll_anabufsel_tx0);
	/* rx low performence 0x45[10][8] = 1*/
	sys_ana_ll_set_ana_reg5_anabufsel_rx1((s_pm_config.btpll_anabufsel_rx1 == 0xff) ? 0 : s_pm_config.btpll_anabufsel_rx1);
	sys_ana_ll_set_ana_reg5_anabufsel_rx0((s_pm_config.btpll_anabufsel_rx0 == 0xff) ? 1 : s_pm_config.btpll_anabufsel_rx0);
...
void sys_hal_exit_low_analog(void)
{
	...
	sys_ana_ll_set_ana_reg3_hpssren((s_pm_config.wifipll_hpssren == 0xff) ? 1 : s_pm_config.wifipll_hpssren);
	/* tx high performence 0x45[11][9] = 3*/
	sys_ana_ll_set_ana_reg5_anabufsel_tx1((s_pm_config.wifipll_anabufsel_tx1 == 0xff) ? 1 : s_pm_config.wifipll_anabufsel_tx1);
	sys_ana_ll_set_ana_reg5_anabufsel_tx0((s_pm_config.wifipll_anabufsel_tx0 == 0xff) ? 1 : s_pm_config.wifipll_anabufsel_tx0);
	/* rx high performence 0x45[10][8] = 3*/
	sys_ana_ll_set_ana_reg5_anabufsel_rx1((s_pm_config.wifipll_anabufsel_rx1 == 0xff) ? 1 : s_pm_config.wifipll_anabufsel_rx1);
	sys_ana_ll_set_ana_reg5_anabufsel_rx0((s_pm_config.wifipll_anabufsel_rx0 == 0xff) ? 1 : s_pm_config.wifipll_anabufsel_rx0);
