...
#define TICK_CNT_100MS (100 * bk_rtc_get_ms_tick_count())
...
__IRAM_SEC uint32_t aon_pmu_hal_get_dpll_unlock(uint32_t *unlockL, uint32_t *unlockH)
{
    aon_pmu_r7d_t r7d;
...
__IRAM_SEC uint32_t aon_pmu_hal_rtc_get_samp_sel(void)
{
	return aon_pmu_ll_get_r2_rtc_value_samp_sel();
}
...
__IRAM_SEC void aon_pmu_hal_r0_latch_to_r7b(void)
{
...
__IRAM_SEC void aon_pmu_hal_set_gpio_sleep(uint32_t value, bool write_immediately)
{
	if (write_immediately) {
		uint32_t r0 = aon_pmu_ll_get_r0();
		uint32_t r7b = aon_pmu_ll_get_r7b();
...
{
	return aon_pmu_ll_get_r7b_app_boot_validate_pending() ? 1u : 0u;
}

void aon_pmu_hal_set_app_boot_validate_pending(uint32_t value)
{
	uint32_t v = (value != 0u) ? 1u : 0u;

#if CONFIG_AON_PMU_REG0_REFACTOR_DEV
	{
		uint32_t r0_save = aon_pmu_ll_get_r0();

		aon_pmu_ll_set_r0(aon_pmu_ll_get_r7b());
		aon_pmu_ll_set_r0_app_boot_validate_pending(v);
		aon_pmu_hal_r0_latch_to_r7b();
		aon_pmu_ll_set_r0(r0_save);
		aon_pmu_ll_set_r0_app_boot_validate_pending(v);
	}
#else
	aon_pmu_ll_set_r0_app_boot_validate_pending(v);
...
__IRAM_SEC void aon_pmu_hal_set_halt_cpu(uint32_t value)
{
...
{
	uint32_t time = aon_pmu_ll_get_r79_rtc_tick_l() / TICK_CNT_100MS;
	aon_pmu_ll_set_r2_saved_time(time);
}

uint32_t aon_pmu_hal_get_por_timing()
{
	uint32_t time = aon_pmu_ll_get_r2_saved_time();
	aon_pmu_ll_set_r2_saved_time(0);
	return time;
}
#else
void aon_pmu_hal_save_time(bool is_startup)
{
	uint32_t time = aon_pmu_ll_get_r79_rtc_tick_l() / TICK_CNT_100MS; // unit=100ms
