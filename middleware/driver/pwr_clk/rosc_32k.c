...
__IRAM_SEC static void rosc_ckmn_isr(void)
{
	...
	bk_rtc_set_clock_freq(s_rosc_freq_hz);
	bk_ckmn_ckest_disable();
exit:
...
__IRAM_SEC bk_err_t bk_rosc_32k_ckest_prog(uint32_t count)
{
	if (0 == bk_ckmn_ckest_check_busy())
	{
		...
		bk_rtc_set_clock_freq(s_rosc_freq_hz); //update base time to compensate the frequency change
#if CONFIG_DEEP_LV
		bk_ckmn_soft_reset();
#endif
...
static void rosc_calib_ckmn_isr(void)
{
	...
	/* avoid GiveFromISR with NULL if thread already deinited semaphore */
	if(s_rosc_calib_sema == NULL) {
		return;
	}
...
static void rosc_calib_thread(void *args)
{
	...
	/* Lock interrupts so CKEST ISR cannot run between deinit and clearing sema (avoids GiveFromISR with NULL) */
