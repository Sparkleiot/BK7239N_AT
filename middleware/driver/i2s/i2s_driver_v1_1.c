...
static bk_err_t i2s_init_channel_legacy(i2s_drv_info_t *drv_info, i2s_channel_id_t chl, i2s_txrx_type_t type, uint32_t buff_size, i2s_data_handle_cb data_handle_cb, RingBufferContext **rb)
{
    ...
    case I2S_CHANNEL_1:
        if (drv_info->chl1_cfg == NULL) {
            chl_cfg = (i2s_chl_cfg_t *)os_zalloc(sizeof(i2s_chl_cfg_t));
            if (chl_cfg == NULL) {
    ...
    if (type == I2S_TXRX_TYPE_TX) {
        txrx_cfg = (i2s_txrx_cfg_t *)os_zalloc(sizeof(i2s_txrx_cfg_t));
        if (txrx_cfg == NULL) {
    ...
    if (type == I2S_TXRX_TYPE_TX) {
        txrx_cfg->buff_addr = (uint8_t *)os_zalloc(buff_size + DMA_CARRY_I2S_RINGBUF_SAFE_INTERVAL);
    } else {
        txrx_cfg->buff_addr = (uint8_t *)os_zalloc(buff_size + DMA_CARRY_I2S_RINGBUF_SAFE_INTERVAL);
    }
    ...
    txrx_cfg->rb = (RingBufferContext *)os_zalloc(sizeof(RingBufferContext));
    if (txrx_cfg->rb == NULL) {
                                  ...
                                  RingBufferContext **rb, i2s_data_handle_cb data_handle_cb)
{
    ...
    *txrx_cfg = (i2s_txrx_cfg_t *)os_zalloc(sizeof(i2s_txrx_cfg_t));
    ...
    (*txrx_cfg)->state = I2S_TXRX_STATE_NULL;
    (*txrx_cfg)->buff_size = buff_size;
    (*txrx_cfg)->buff_addr = (uint8_t *)os_zalloc(alloc_size);
    if ((*txrx_cfg)->buff_addr == NULL) {
        I2S_LOGE("malloc buff_addr fail \n");
        os_free(*txrx_cfg);
        *txrx_cfg = NULL;
                                              ...
                                              i2s_txrx_type_t type, i2s_data_handle_cb data_handle_cb)
{
    ...
    transfer_len = (type == I2S_TXRX_TYPE_TX) ? 
        (txrx_cfg->buff_size + 0) / 2 : /* DMA_CARRY_I2S_RINGBUF_SAFE_INTERVAL*/
...
bk_err_t bk_i2s_driver_init(void)
{
    ...
    sys_drv_apll_en(1); /* TODO:wangzhilei*/
    ...
    switch (id) {
        case I2S_GPIO_GROUP_0:
            ...
            if (i2s_drv_info == NULL) {
                i2s_drv_info = (i2s_drv_info_t *)os_zalloc(sizeof(i2s_drv_info_t));
                if (!i2s_drv_info) {
                    return BK_FAIL;
                }
            }
        ...
        case I2S_GPIO_GROUP_1:
            ...
            if (i2s1_drv_info == NULL) {
                i2s1_drv_info = (i2s_drv_info_t *)os_zalloc(sizeof(i2s_drv_info_t));
                if (!i2s1_drv_info) {
                    return BK_FAIL;
                }
            }
        ...
        case I2S_GPIO_GROUP_2:
            ...
            if (i2s2_drv_info == NULL) {
                i2s2_drv_info = (i2s_drv_info_t *)os_zalloc(sizeof(i2s_drv_info_t));
                if (!i2s2_drv_info) {
                    return BK_FAIL;
                }
            }
