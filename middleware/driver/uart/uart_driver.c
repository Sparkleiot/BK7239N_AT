...
static void uart_init_gpio(uart_id_t id)
{
	switch (id)
	{
		case UART_ID_0:
		{
			...
			bk_gpio_set_value(GPIO_10, 0);
			bk_gpio_set_value(GPIO_11, 0);
#endif
...
__IRAM_SEC bk_err_t uart_write_byte(uart_id_t id, uint8_t data)
{
...
__IRAM_SEC bk_err_t uart_write_ready(uart_id_t id)
{
	/* wait for fifo write ready
	 * optimize it when write very fast
	 * wait for fifo write ready will waste CPU performance
...
__IRAM_SEC static bk_err_t uart_pm_restore(uint64_t sleep_time, void *args)
{
	uart_id_t uart_id = (uart_id_t)args;
...
__IRAM_SEC bk_err_t bk_uart_pm_backup(uart_id_t id)
{
	uart_pm_backup(0, (void *)id);
	return BK_OK;
}

__IRAM_SEC bk_err_t bk_uart_pm_restore(uart_id_t id)
{
	uart_pm_restore(0, (void *)id);
	return BK_OK;
}
...
bk_err_t bk_uart_driver_init(void)
{
	...
	{
#if (0 == CONFIG_SOC_BK7236XX) && (0 == CONFIG_SOC_BK7239XX)
...
static void uart_tx_dma_write_done(dma_id_t dma_id)
{
	...
	/* Find uart_id by dma_id and release semaphore */
	for (uart_id_t id = UART_ID_0; id < SOC_UART_ID_NUM_PER_UNIT; id++) {
		if (s_uart[id].tx_dma_enable && s_uart[id].tx_dma_id == dma_id) {
			if (s_uart_sema[id].tx_dma_sema) {
				rtos_set_semaphore(&s_uart_sema[id].tx_dma_sema);
			}
			break;
		}
	}
...
static bk_err_t uart_tx_dma_init(uart_id_t id)
{
	...
	/* Create semaphore for TX DMA sync */
	if (s_uart_sema[id].tx_dma_sema == NULL) {
		bk_err_t ret = rtos_init_semaphore(&s_uart_sema[id].tx_dma_sema, 1);
		if (ret != BK_OK) {
			UART_LOGE("uart(%d) tx_dma_sema init failed\r\n", id);
			return BK_FAIL;
		}
	}
...
static bk_err_t uart_tx_dma_deinit(uart_id_t id)
{
	...
	/* Deinit semaphore */
	if (s_uart_sema[id].tx_dma_sema) {
		rtos_deinit_semaphore(&s_uart_sema[id].tx_dma_sema);
		s_uart_sema[id].tx_dma_sema = NULL;
	}
...
#if (CONFIG_ATE_TEST && CONFIG_DEEP_LV)
	if (id == UART_ID_0) {
...
__IRAM_SEC bk_err_t bk_uart_enable_tx_interrupt(uart_id_t id)
{
	UART_RETURN_ON_NOT_INIT();
	UART_RETURN_ON_INVALID_ID(id);
	UART_PM_CHECK_RESTORE(id);
...
__IRAM_SEC bk_err_t bk_uart_disable_tx_interrupt(uart_id_t id)
{
	UART_RETURN_ON_NOT_INIT();
	UART_RETURN_ON_INVALID_ID(id);
	UART_PM_CHECK_RESTORE(id);
...
__IRAM_SEC bk_err_t bk_uart_enable_rx_interrupt(uart_id_t id)
{
	UART_RETURN_ON_NOT_INIT();
	UART_RETURN_ON_INVALID_ID(id);
	UART_PM_CHECK_RESTORE(id);
...
bk_err_t bk_uart_write_bytes(uart_id_t id, const void *data, uint32_t size)
{
	...
	if(s_uart[id].tx_dma_enable) {
		...
		if (s_uart_sema[id].tx_dma_sema) {
			bk_err_t ret = rtos_get_semaphore(&s_uart_sema[id].tx_dma_sema, BEKEN_WAIT_FOREVER);
			if (ret != BK_OK) {
				UART_LOGE("uart(%d) wait tx_dma_sema failed\r\n", id);
				return BK_FAIL;
			}
		}
	} else
...
__IRAM_SEC bk_err_t bk_uart_set_enable_rx(uart_id_t id, bool enable)
{
	UART_RETURN_ON_NOT_INIT();
	UART_RETURN_ON_INVALID_ID(id);
	UART_PM_CHECK_RESTORE(id);
...
__IRAM_SEC bk_err_t bk_uart_set_enable_tx(uart_id_t id, bool enable)
{
	UART_RETURN_ON_NOT_INIT();
	UART_RETURN_ON_INVALID_ID(id);
	UART_PM_CHECK_RESTORE(id);
...
__IRAM_SEC bool bk_uart_is_tx_over(uart_id_t id)
{
	return uart_hal_is_tx_fifo_empty(&s_uart[id].hal, id);
}

__IRAM_SEC void bk_uart_wait_tx_over(uart_id_t id)
{
 ...
 * clear int status
 */
__IRAM_SEC static void uart_isr_common(uart_id_t id)
{
...
__IRAM_SEC void uart0_isr(void)
{
