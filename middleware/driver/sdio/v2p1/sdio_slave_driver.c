...
__attribute__((section(".iram_sec_code"))) static void request_dma_copy_packet_data_to_sdio(void)
{
	...
	#if 1 //CONFIG_SDIO_CMD_AUTO_RSP_EN   //7236N ASIC BUG fixed: one packet send all data
...
__attribute__((section(".iram_sec_code"))) void sdio_slave_dma_rx_finish(dma_id_t dma_id)
{
	...
	#if CONFIG_SDIO_SLAVE_RX_WITH_BLK
	//sdio_hal_set_dat_s_rd_mul_blk(1);   //release data0 busy status, and output high       NUM1
	s_rx_blk_cnt++;
	...
	if((s_rx_last_len >= s_pack_rx_left_len) || (s_rx_last_len >= SDIO_RX_MEMCPY_PACK_SIZE))
	{
		...
		if(s_pack_rx_left_len)
		{
			request_dma_copy_sdio_data_to_payloads();
				...
				sdio_hal_set_dat_s_rd_mul_blk(1);   //release data0 busy status, and output high    NUM2
			#endif
		}
		...
		request_dma_copy_sdio_data_to_payloads();
		#if !CONFIG_SOC_BK7236
		sdio_hal_set_dat_s_rd_mul_blk(1);   //release data0 busy status, and output high    NUM3
		#endif
	}
#endif    //end of CONFIG_SDIO_SLAVE_RX_WITH_BLK
}
 ...
 * @return NONE
 */
#define REG_GPIO_P4                      *((volatile unsigned long *) (0x44830000+0x44*4))

static inline void sdio_slave_set_notify_host(void)
{
	SDIO_LOG_DEBUG_FUNCTION_ENTRY();

	REG_GPIO_P4 = 0x2000002;
...
bk_err_t sdio_init_gpio(void)
{
	const sdio_gpio_map_t sdio_gpio_map_table[] = SDIO_GPIO_MAP;

	/* set gpio sdio host map: clk,cmd,data0 */
	for (uint32_t i = SDIO_GPIO_CLK_INDEX; i < SDIO_GPIO_PIN_NUMBER; i++) {
		gpio_dev_unmap(sdio_gpio_map_table[i].gpio_id);
	}
	...
	/* sdio host clk */
	bk_gpio_pull_up(sdio_gpio_map_table[SDIO_GPIO_CLK_INDEX].gpio_id);
	bk_gpio_set_capacity(sdio_gpio_map_table[SDIO_GPIO_CLK_INDEX].gpio_id, 3);
#if CONFIG_IO_MATRIX_VER2_0
	bk_iomx_set_gpio_func(sdio_gpio_map_table[SDIO_GPIO_CLK_INDEX].gpio_id, FUNC_CODE_SDIO_CLK);
#endif

	/* sdio host cmd */
	bk_gpio_pull_up(sdio_gpio_map_table[SDIO_GPIO_CMD_INDEX].gpio_id);
	bk_gpio_set_capacity(sdio_gpio_map_table[SDIO_GPIO_CMD_INDEX].gpio_id, 3);
#if CONFIG_IO_MATRIX_VER2_0
	bk_iomx_set_gpio_func(sdio_gpio_map_table[SDIO_GPIO_CMD_INDEX].gpio_id, FUNC_CODE_SDIO_CMD);
#endif

	/* sdio host data0 */
	bk_gpio_pull_up(sdio_gpio_map_table[SDIO_GPIO_DATA0_INDEX].gpio_id);
	bk_gpio_set_capacity(sdio_gpio_map_table[SDIO_GPIO_DATA0_INDEX].gpio_id, 3);
#if CONFIG_IO_MATRIX_VER2_0
	bk_iomx_set_gpio_func(sdio_gpio_map_table[SDIO_GPIO_DATA0_INDEX].gpio_id, FUNC_CODE_SDIO_DATA_0);
#endif

#if CONFIG_SDIO_4LINES_EN
	/* sdio host data1,data2,data3 */
	for (uint32_t i = SDIO_GPIO_DATA1_INDEX; i < SDIO_GPIO_PIN_NUMBER; i++) {
		bk_gpio_pull_up(sdio_gpio_map_table[i].gpio_id);
		bk_gpio_set_capacity(sdio_gpio_map_table[i].gpio_id, 3);
	}

#if CONFIG_IO_MATRIX_VER2_0
	bk_iomx_set_gpio_func(sdio_gpio_map_table[SDIO_GPIO_DATA1_INDEX].gpio_id, FUNC_CODE_SDIO_DATA_1);
	bk_iomx_set_gpio_func(sdio_gpio_map_table[SDIO_GPIO_DATA2_INDEX].gpio_id, FUNC_CODE_SDIO_DATA_2);
	bk_iomx_set_gpio_func(sdio_gpio_map_table[SDIO_GPIO_DATA3_INDEX].gpio_id, FUNC_CODE_SDIO_DATA_3);
...
bk_err_t sdio_deinit_gpio(void)
{
	const sdio_gpio_map_t sdio_gpio_map_table[] = SDIO_GPIO_MAP;
		...
		if (kNoErr == ret)
		{
			...
			switch(msg.id)
			{
				...
				case SDIO_NOTIFY_START_READ:
				{
					...
					//bk_gpio_set_value(GPIO_ID_SDIO_NOTIFY_START_READ, GPIO_ENTER_LEVEL);
					...
					request_cpu_and_dma_copy_sdio_data_to_payloads();
#endif
					//bk_gpio_set_value(GPIO_ID_SDIO_NOTIFY_START_READ, GPIO_EXIT_LEVEL);
					break;
				}

#if 0  //cpu read fifo to do
				case SDIO_READ_TO_FIFO_FINISH:
				{
					//bk_gpio_set_value(GPIO_ID_SDIO_READ_TO_FIFO_FINISH, GPIO_ENTER_LEVEL);
					if  (read_to_fifo_flag == 0) {
						request_cpu_copy_sdio_data_to_payloads();
						read_to_fifo_flag = 1;
					}
					//bk_gpio_set_value(GPIO_ID_SDIO_READ_TO_FIFO_FINISH, GPIO_EXIT_LEVEL);
				...
				{
					//bk_gpio_set_value(GPIO_ID_SDIO_READ_PACK_FINISH, GPIO_ENTER_LEVEL);
					...
					//bk_gpio_set_value(GPIO_ID_SDIO_READ_PACK_FINISH, GPIO_EXIT_LEVEL);
				...
				case SDIO_WRITE_BLK_FINISH:
				{
					...
					if(write_blk_cnt == 0)
					{
#if 0	//move to top-half to deal it, let it quickly enable CMD START, and then can response host next CMDx
					...
					if(cur_pack_left_size < SDIO_ALIGN_LENGTH(payload_len))	//Current pack hasn't enough buffer
					{
						...
						while(s_sdio_driver.pack.wr_index - s_sdio_driver.pack.rd_index >= SDIO_MEMCPY_PACK_CNT)
						{
							...
							if(delay_ms_cnt > SDIO_TX_PACK_FULL_TIMEOUT) {
								//BK_ASSERT(0);
								SDIO_LOG_ERR("sdio tx timeout, retry cnt = %d.\r\n", delay_ms_cnt);
								break;
							}
						}
...
#else
				sdio_cmd52_arg_t cmd52_arg;
				uint32_t cmd52_arg_value = 0;
				cmd52_arg.v = cmd52_arg_value = sdio_hal_slave_get_cmd_arg0();

				//host read block cnt
				if((cmd52_arg.rw_flag == 0) && (cmd52_arg.reg_addr == 0))	//host read reg0 ==>Slave TX,notify host how many blocks needs to read
				{
					sdio_hal_slave_set_tx_length(0);	//clear previous tx length
					sdio_hal_slave_cmd_start(1);	//wait to receive next CMD.
				}
				else if((cmd52_arg.rw_flag == 0) && (cmd52_arg.reg_addr == 4))	//host read reg1 ==>flow control, how many payloads slave RX have
				{
					//TODO:
					sdio_hal_slave_cmd_start(1);	//wait to receive next CMD.
				}
...
static bk_err_t sdio_sw_init(void)
{
	...
	{
		s_sdio_driver.pack.end_addr[i] = s_sdio_driver.pack.start_addr[i] = os_malloc(SDIO_MEMCPY_PACK_SIZE);
		if(s_sdio_driver.pack.start_addr[i] == NULL)
		{
...
static bk_err_t sdio_slave_hw_init(void)
{
	...
	//dma
	//v2p1 has to enable DMA to TX/RX data to increase throughput
