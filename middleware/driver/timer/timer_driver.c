...
bk_err_t bk_timer_driver_init(void)
{
    ...
    bk_timer_clock_select(TIMER_ID0, TIMER_SCLK_XTAL);
#if (SOC_TIMER_INTERRUPT_NUM > 1)
    bk_timer_clock_select(TIMER_ID3, TIMER_SCLK_XTAL);
#endif
...
__IRAM_SEC uint32_t timer_clear_isr_status(void)
{
	uint32_t int_status;
...
__IRAM_SEC static void timer_isr(void)
{
    uint32_t int_status;
    timer_hal_t *hal = &s_timer.hal;
