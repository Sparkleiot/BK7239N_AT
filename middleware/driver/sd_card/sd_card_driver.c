...
static bk_err_t sd_card_cmd_go_idle_state(void)
{
	...
	if (error_state != BK_OK) {
		/*Pay attention to SDIO module enable and its power LDO*/
...
static bk_err_t sd_card_power_on(void)
{
	...
	if ((response & SD_OCR_HIGH_CAPACITY) == SD_OCR_HIGH_CAPACITY) {
		/*Pay attention to pull-up resistors and the relevant jumpers on the EVB*/
...
#if (CONFIG_SDCARD_BUSWIDTH_4LINE != CONFIG_SDIO_4LINES_EN)
#error "CONFIG_SDCARD_BUSWIDTH_4LINE != CONFIG_SDIO_4LINES_EN"
#endif
