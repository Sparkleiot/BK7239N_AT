...
static uint32_t g_is_master_role = 0;
...
spi_driver_t *spi_get_drv_object(spi_id_t id)
{
        spi_driver_t *spi_obj;
...
#if CONFIG_SUPPORT_XMIT_READY_CALLBACK
bk_err_t bk_spi_register_xmit_ready_cb(spi_id_t id, FUNCPTR ready_cb)
{
        spi_driver_t *spi_obj = spi_get_drv_object(id);

        spi_obj->xmit_ready_cb = ready_cb;
...
#if CONFIG_SUPPORT_SPI_ISR_HIGH_PRIORITY
                arch_interrupt_set_priority(s_spi[id].isr_src_id, IQR_PRI_HIGH);
#endif
        }
        spi_statis_init();
...
#if CONFIG_SUPPORT_XMIT_READY_CALLBACK
        spi_driver_t *spi_obj = spi_get_drv_object(id);
        spi_obj->xmit_ready_cb = config->ready_cb;
#endif
        if(SPI_ROLE_MASTER == config->role) {
                g_is_master_role = 1;
        } else {
                g_is_master_role = 0;
        }
...
bk_err_t bk_spi_read_bytes(spi_id_t id, void *data, uint32_t size)
{
        ...
        int_level = spi_enter_critical();
        if(s_spi[id].rx_offset < s_spi[id].rx_size){
                SPI_LOGE("[SPI%d] bk_spi_read_bytes failed: rx incomplete! rx_offset(%d) < rx_size(%d), missing %d bytes\r\n", 
                         id, s_spi[id].rx_offset, s_spi[id].rx_size, s_spi[id].rx_size - s_spi[id].rx_offset);
                spi_exit_critical(int_level);
                return BK_FAIL;
        }
        bk_spi_clr_rx(id);
...
bk_err_t bk_spi_dma_duplex_xfer(spi_id_t id, const void *tx_data, uint32_t tx_size, void *rx_data, uint32_t rx_size)
{
        ...
        while(len > 0) {
                ...
                if(tx_data) {
                        ...
                        for (int i = 0; i < (tx_size > 48 ? 48 : tx_size); i++) {
                                data = (uint32_t)tx_buf[i];
                                BK_WHILE (!spi_hal_is_tx_fifo_wr_ready(&s_spi[id].hal));
                                spi_hal_write_byte(&s_spi[id].hal,data);
                        }
                        offset = tx_size > 48 ? 48 : tx_size;
...
#if CONFIG_SUPPORT_XMIT_READY_CALLBACK
        FUNCPTR spi_ready_cb;
        spi_driver_t *spi_obj = spi_get_drv_object(id);
        spi_ready_cb = spi_obj->xmit_ready_cb;
#endif
        while(left_len > 0) {
                ...
                //wait additional time to ensure all data in FIFO and shift register are sent out
                 bk_delay_us(48);
...
uint32_t bk_spi_dma_read_bytes(spi_id_t id, void *data, uint32_t size)
{
        ...
        if (buf_offset != size) {
                SPI_LOGE("DMA read bytes mismatch: buf_offset=%d, size=%d\r\n", buf_offset, size);
                return buf_offset;
        }
...
static void spi_isr_common(spi_id_t id)
{
        ...
        if (spi_hal_is_rx_finish_int_triggered(hal, int_status)) {
                ...
                if (s_spi[id].is_rx_blocked) {
                        if (0 == s_spi[id].rx_offset) {
                                SPI_LOGW("rx finish int triggered but rx_offset is 0\r\n");
                        }
                        if (s_spi[id].rx_offset < s_spi[id].rx_size) {
                                SPI_LOGE("rx finish int triggered but rx_offset(%d) < rx_size(%d)\r\n", s_spi[id].rx_offset, s_spi[id].rx_size);
                        }
