...
static uint32_t s_start_type = RESET_SOURCE_POWERON;
static uint32_t s_misc_value_save = RESET_SOURCE_POWERON;
static uint32_t s_reset_reason_inited = false;
...
#elif CONFIG_GPIO_WAKEUP_SUPPORT
        BK_LOGI(TAG, "by gpio - %d\r\n", bk_gpio_get_wakeup_gpio_id());
#endif
    }
    BK_LOGI(TAG, "regs - start_type: %x, misc_value_save: %x\r\n", s_start_type, s_misc_value_save);
}
...
uint32_t reset_reason_deep_sleep_check(void)
{
    ...
    /* FIXME: something wrong with the wakeup source,
     * so we use the ana rtc/ana gpio int status to represent the wakeup source
     */
    if (misc_value == 0) {
        if (sys_hal_get_boot_ana_rtc_int_status() == 1) {
            misc_value = WAKEUP_SOURCE_RTC;
        } else if (sys_hal_get_boot_ana_gpio_int_status() == 1) {
            misc_value = WAKEUP_SOURCE_GPIO;
        } else {
            BK_LOGW(TAG, "Unknown wakeup source\r\n");
        }
    }
#endif
    ...
    case WAKEUP_SOURCE_RTC:
        ...
        break;
    case WAKEUP_SOURCE_TOUCH:
        misc_value = RESET_SOURCE_DEEPPS_TOUCH;
        break;
    case WAKEUP_SOURCE_USB:
...
void bk_misc_set_reset_reason(uint32_t type)
{
    ...
    if (type == RESET_SOURCE_POWERON ||
        type == RESET_SOURCE_WATCHDOG ||
        type == RESET_SOURCE_ILLEGAL_PERSIST_DATA) {
...
#else
#error "the soc type doesnot be support at the reset reason module"
#endif
