...
#if ((CONFIG_SDIO_V2P0 && CONFIG_SDIO_SLAVE) || CONFIG_SDIO_V2P1 || CONFIG_SDIO_V2P2)
...
extern void os_print_memory_config_boot_summary(void);

static void driver_print_boot_summary(void)
{
#if CONFIG_SHELL_ASYNCLOG
	int old_level = shell_get_log_level();
	shell_set_log_level(BK_LOG_INFO);
#endif

	os_print_memory_config_boot_summary();

#if CONFIG_FLASH
	bk_printf_ext(BK_LOG_INFO, "flash", "ID=0x%X\r\n", bk_flash_get_id());
#endif

#if CONFIG_SHELL_ASYNCLOG
	shell_set_log_level(old_level);
#endif
}
...
int driver_init(void) {
	...
	//Important notice!
	//it must be called after uart is ready and before os_sys_tick start running
	aon_pmu_driver_dump_time();
#endif
	...
	driver_print_boot_summary();

#if CONFIG_EASY_FLASH
	easyflash_init();
#endif

#if CONFIG_AON_RTC || CONFIG_ANA_RTC //Move the rtc init here for getting the more time to caculate the tick of rtc at the bellow.
	bk_aon_rtc_driver_init();
#if CONFIG_AUTO_DETECT_32K_SOURCE  /*Get the rtc tick, when enable the auto detect 32k, At the begining will config the external 32K source*/
	uint32_t pre_tick = bk_aon_rtc_get_current_tick(AON_RTC_ID_1);
#endif
#endif
