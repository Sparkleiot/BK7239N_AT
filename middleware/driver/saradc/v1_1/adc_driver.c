...
static bk_err_t adc_set_default_cali_val(adc_buf_t *data)
{
...
bk_err_t bk_adc_driver_init(void)
{
    ...
    adc_set_default_cali_val(&s_adc_buf);
...
float saradc_calculate(UINT16 adc_val)
{
    ...
    practic_voltage = ((adc_val - cali_low_val) * 1.8);
    practic_voltage = (practic_voltage / (cali_high_val - cali_low_val)) + 0.2;
#else
...
float temprature_calculate(UINT16 adc_val)
{
    ...
    #if (CONFIG_SOC_BK7236XX) && (!CONFIG_SOC_BK7236N) && (!CONFIG_SOC_BK7259)
...
float bk_adc_data_calculate(UINT16 adc_val, UINT8 adc_chan)
{
    ...
    else if (adc_chan == ADC_7) {
        return temprature_calculate(adc_val);
    }
    else if(adc_chan == 8 || adc_chan == 9 || adc_chan == 11)
    {
...
UINT32 saradc_set_calibrate_val(uint16_t *value, SARADC_MODE mode)
{
    ...
    if (mode == SARADC_CALIBRATE_LOW) {
        ...
        if ((*value + threshold < dft_value) || (dft_value + threshold < *value)) {
            s_adc.adc_cali_invalid = 1;
        }
        if (!s_adc.adc_cali_invalid) {
