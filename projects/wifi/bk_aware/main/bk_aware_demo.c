...
#define BK_AWARE_USE_WHITE_LIST 1
#define BK_AWARE_DEFAULT_OUI  "\xC8\x47\x8C"

#if BK_AWARE_USE_WHITE_LIST
typedef struct {
	uint8_t type;
	uint8_t oui[3];
} filter_cfg_t;

static filter_cfg_t bk_aware_white_list[] = {
	{0xD0, "\xC8\x47\x8C"},
};
#endif
...
#if BK_AWARE_USE_WHITE_LIST
static void example_bk_aware_add_white_list(void)
{
	int i;
	uint8_t num = sizeof(bk_aware_white_list)/sizeof(filter_cfg_t);

	for (i = 0; i < num; i++) {
		bk_rlk_add_white_list(bk_aware_white_list[i].type, bk_aware_white_list[i].oui);
	}
}
#endif
...
static bk_err_t example_bk_aware_recv_cb(bk_rlk_recv_info_t *rx_info)
{
	...
	#if BK_AWARE_USE_WHITE_LIST
	if (rx_info->data[0] != 0x7F || os_memcmp(&rx_info->data[1], bk_aware_white_list[0].oui, 3)) {
		return BK_FAIL;
	}
	#endif
...
//	assert(send_param->len >= sizeof(example_bk_aware_data_t));
	buf->pid = 0x7F;
	#if BK_AWARE_USE_WHITE_LIST
	os_memcpy(buf->oui, bk_aware_white_list[0].oui, 3);
	#else
	os_memcpy(buf->oui, BK_AWARE_DEFAULT_OUI, 3);
	#endif
	buf->random = bk_rand();
	buf->vendor[0] = 0xDD;
	buf->vendor[1] = 0x0F;
	#if BK_AWARE_USE_WHITE_LIST
	os_memcpy(&buf->vendor[2], bk_aware_white_list[0].oui, 3);
	#else
	os_memcpy(&buf->vendor[2], BK_AWARE_DEFAULT_OUI, 3);
	#endif
	buf->vendor[5] = 0x04;
	buf->vendor[6] = 0x01;
...
static void example_bk_aware_task(void *pvParameter)
{
	...
	#if BK_AWARE_USE_WHITE_LIST
	if ((err = bk_rlk_send_by_oui(send_param->dest_mac, send_param->buffer, send_param->len,
							bk_aware_white_list[0].type, bk_aware_white_list[0].oui)) < BK_OK)
	#else
...
static bk_err_t example_bk_aware_init(void)
{
	...
	bk_rlk_init();
	bk_rlk_set_channel(CONFIG_BK_AWARE_CHANNEL, 0);
	#if BK_AWARE_USE_WHITE_LIST
	bk_rlk_use_legacy_mac_header(1);
	#endif
...
void bk_aware_demo_main(void)
{
	...
	bk_rlk_set_ps_window(90);
	bk_rlk_set_ps_interval(200);
	#if BK_AWARE_USE_WHITE_LIST
	example_bk_aware_add_white_list();
	#endif
	example_wifi_init();
