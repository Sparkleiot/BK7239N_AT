...
本文档以 BK7239 开发板为例，通过一个简单的示例项目向您展示:
...
BK7239 采用 22 nm 工艺制成，具有最佳的功耗性能，射频性能，稳定性，通用性，可靠性，
和最高级别的安全（PSA Level 2），适用于各种应用场景和不同功耗，安全需求。

博通集成为用户提供完整的软，硬件资源，进行 BK7239 硬件设备开发。其中，软件开发环境
...
然后切换到稳定分支Tag节点, 如v2.2.1.1::

    git checkout -B your_branch_name v2.2.2.1
...
您可以通过工程配置文件来进行更改 Armino 默认配置或者针对不同芯片进行差异化配置::
    ...
    如： bk7239n/config >> bk7239n.defconfig >> KConfig
    + 工程配置文件示例：
...
app工程在编译完成后，在build/app/bk7239n目录下生成all-app.bin，使用此bin文件烧录即可。安全工程首次烧录时，需要先烧录bootloader.bin，再烧录all-app.bin。
...
.. figure:: ../../../common/_static/download_uart_bk7239n.png
    :align: center
    :alt: BKFIL GUI
    :figclass: align-center
