...
可以将这些文件归为两类：
 ...
 - 配置修改文件:
   ...
   - middleware/soc/bk7239n.defconfig - Armino 提供的 BK7239 默认配置。
   - my_projects/config/common/config - 工程中不同 BK72xx 共享的配置。
   - my_projects/config/bk7239n/config - 工程中 BK7239 专有配置。
...
在 Armino 工程中，Kconfig 配置优先级为:
  ...
  my_projects/config/bk7239n/config > my_projects/config/common/config > bk7239n.defconfig > 配置定义文件
