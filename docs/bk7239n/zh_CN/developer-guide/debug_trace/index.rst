...
Armino平台BK7239系统调试命令
--------------------------------------


 - BK7239 CPU的log通过串口DL_UART0输出(默认波特率为115200)
 ...
 - 通过串口输入help命令查看当前支持的调试命令：
  ...
  ::
    ...
    dvfs: dvfs [cksel_core] [ckdiv_core] [ckdiv_bus] [ckdiv_cpu0]
    ...
    setcpufreq: setcpufreq [ckdiv_core] [ckdiv_bus] [ckdiv_cpu0]
    setjtagmode: set swd mode {cpu0} {group1|group2}
...
--------------------------------------
 ...
 - Eclipse工程配置
  ...
  .. figure:: ../../../../common/_static/bk7236_jlink_config.png
      :align: center
      :alt: BK7239 JLink configuration
 ...
 - 默认swd连接cpu0，BK7239有两个swd口(grou1/group2)
 - 可以通过setjtagmode cpu0 group1命令设置swd连接cpu0
 - 可以通过jtagmode命令查看当前swd状态


Armino平台BK7239异常dump一键恢复现场工具
------------------------------------------------
 ...
 - BK7239 dump工具常见问题:
   ...
   + cpu需要当前cpu的寄存器, itcm, dtcm, sram加上elf就可以恢复现场
