...
- 1. For better maintenance release, start from v2.0.1.8: The packaging path of the bootloader image is changed from the original default path middleware/boards/ to the components/bk_libs directory.
  ...
  -
    - 1. When changing the partition by modifying the file,the bk7239n_partitions.csv path is: bk_idk/projects/app/config/bk7239n;
    - 2. Directly make bk7239n;
    - 3. No need to modify the components/bk_libs/bk7239n/bootloader/normal_bootloader/bootloader.bin;
  - 2. This section uses the customization/config_ab project in bk_avdk as an example to modify partitions:
  -
    - 1. When changing the partition by modifying the file,the bk7239n_partitions csv path is: projects/customization/config_ab/config/bk7239n/;
    - 2. Directly make bk7239n PROJECT=customization/config_ab;
    - 3. No need to modify the components/bk_libs/bk7239n/bootloader/normal_bootloader/bootloader.bin;
  ...
  -
    - the bootloader original path is：middleware/boards/bk7239n/bootloader.bin;
    - bootloader path for now: the components/bk_libs/bk7239n/bootloader/normal_bootloader/bootloader.bin;
  - 2. Take the customization/config_ab project of 7236 as an example:
  -
    - bootloader path for now: the components/bk_libs/bk7239n/bootloader/ab_bootloader/bootloader.bin;
