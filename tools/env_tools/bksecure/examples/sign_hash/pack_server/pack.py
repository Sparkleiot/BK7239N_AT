...
def pack_server_get_app_bin_hash(tool_path):
    ...
    return [bl2_bin_hash, None, app_bin_hash]

# def pack_server_sign_from_app_sig(tool_path, bl2_sig_r, bl2_sig_s, bl2_b_sig_r, bl2_b_sig_s, app_sig):
def pack_server_sign_from_app_sig(tool_path, bl2_sig_r, bl2_sig_s, app_sig):
    logging.debug(f'')
    ...
    cmd = f'{tool_path}/main.py steps sign_from_app_sig --bl2_sig_r {bl2_sig_r} --bl2_sig_s {bl2_sig_s} --app_sig {app_sig} --debug'
    run_cmd(cmd)
    save_cmd("", cmd)
