...
def decrypt(args):
    ...
    if '.csv' in args.output:
        outfile=outfile.replace('.csv','.bin')  
    # Check if key file has .bin extension
    input_files = [args.input, args.key,outfile]
    ...
    args.outdir, outfile = set_target_filepath(args.outdir, outfile)

    output_buf = bytearray(b'\xff')

    with open(args.input, 'rb') as input_file, open(outfile,'wb') as output_file:
        while True:
    ...
    print('\nCreated NVS decrypted binary: ===>', outfile)
    if '.csv' in args.output:                
        convert_csv(outfile,args.output)
...
def convert_csv(infile:str,outfile:str):
    try:
    ...
    nvs = nvs_parser.NVS_Partition(bytearray(partition))
    csv_buf = []
    try:
        with open(outfile, 'wt',encoding='utf-8',newline='') as wf:
            csv_w = csv.writer(wf)           
            for page in nvs.pages:
                if (not page.is_empty) and (not page.entries_empty):
                    csv_buf = ['#Sample csv file']
                    csv_w.writerow(csv_buf)
                    csv_buf = ['key','type','encoding','value']
                    csv_w.writerow(csv_buf)
                    for entry in page.entries:
                        if not entry.is_empty:
                            if 'Namespace' in entry.key:
                                csv_buf=[entry.key,'namespace']
                            else:
                                if entry.raw[1]<0x20:
                                    csv_buf=[entry.key,entry.namespace,entry.entry_type,entry.childdata]
                                elif entry.raw[1]==0x21:
                                    csv_buf=[entry.key,entry.namespace,entry.entry_type,entry.childdata.decode()]
                                elif entry.raw[1]==0x42:
                                    if 'Base64' in entry.key:
                                        csv_buf=[entry.key,entry.namespace,entry.entry_type,binascii.b2a_base64(entry.childdata).decode()]
                                    elif 'Hex2Bin' in entry.key:
                                        csv_buf=[entry.key,entry.namespace,entry.entry_type,binascii.b2a_hex(entry.childdata).decode()]
                                    elif 'Binary' in entry.key:
                                        csv_buf=[entry.key,entry.namespace,entry.entry_type,hex(entry.childdata)]
                                else:
                                    continue                                                                   
                            csv_w.writerow(csv_buf)
        ...
        print('Bad filename\n')
        raise
    print('\nCreated NVS decrypted csv: ===>', outfile)
