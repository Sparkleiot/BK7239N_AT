...
class NVS_Constants:
    ...
    def __init__(self) -> None:
        ...
        self.item_type = {
            ...
            0x18: 'i64',
            0x21: 'string',
            0x41: 'blob',   #['hex2bin', 'binary', 'base64']
            0x42: 'blob_data', 
            0x48: 'blob_index', 
        }
        self.page_status = {
...
class NVS_Page:
    def __init__(self, page_data: bytearray, address: int):
        ...
        # Load header
        self.page_crc_original = int.from_bytes(page_data[28:32], byteorder='little')
        self.page_crc_computed = crc32(page_data[4:28], 0xFFFFFFFF)
        self.page_index = int.from_bytes(page_data[4:8], byteorder='little')
        ...
        self.entry_states = self.entry_states[:-2]
        self.entries_empty=True
        ...
        while i < int(
            ...
            entry = NVS_Entry(
                ...
                entry_state=self.entry_states[i - 2],
            )
            ...
            if span != 1:
                for span_idx in range(1, span):
                    ...
                    child_entry = NVS_Entry(
                        ...
                        entry_state=self.entry_states[entry_idx],
                    )
...
class NVS_Entry:
    def __init__(self, index: int, entry_data: bytearray, entry_state: str):
        ...
        self.namespace = 'data'
        self.entry_type = nvs_const.item_type.get(self.raw[1], f'0x{self.raw[1]:02x}')
        self.span = self.raw[2]    
        self.chunk_index = self.raw[3]
        self.crc_original = int.from_bytes(self.raw[4:8], byteorder='little')
        self.crc_computed = crc32(self.raw[:4] + self.raw[8:32], 0xFFFFFFFF),

        self.crc_childdata_original = int.from_bytes(self.raw[28:32], byteorder='little')
        key = self.raw[8:24]
        ...
        if self.key is None:
            ...
            self.data = item_convert(self.raw[1], self.raw[24:32])
        self.crc_childdata_computed=0
        self.childdata = []
        if self.data:
            if self.data['value'] is not None:
                self.childdata = self.data['value']
    ...
    def compute_crc(self) -> None:
        ...
        self.crc_data_computed = crc32(children_data, 0xFFFFFFFF)
        self.childdata = children_data
